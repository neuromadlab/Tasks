...
Shuffle_Cond_Version = 3; %1=behav&food / 2=behav&NF / 3=fMRI&food&NF 
...
if (Shuffle_Cond_Version == 1 || Shuffle_Cond_Version == 2)
    ...
    if create_exp_files == 1 
        ...
        % Only one condition file for whole sample in fMRI version
        n_subj = 20;
    end
 ...
 %% Randomly Assign images to blocks   
      ...
      for ind_t_ima = 1:length(images)
        ...
        if BEDVAR_NF == 1
           ...
           block_mat([26,22],4) =  block_mat([22,26],4);
           block_mat([29,21],4) =  block_mat([21,29],4);
           block_mat([27,55],4) =  block_mat([55,27],4);
           block_mat([30,60],4) =  block_mat([60,30],4);

           
           assign_image_mat([26,22]) =  assign_image_mat([22,26]);
           assign_image_mat([29,21]) =  assign_image_mat([21,29]);
           assign_image_mat([27,55]) =  assign_image_mat([55,27]);
           assign_image_mat([30,60]) =  assign_image_mat([60,30]);

           assign_image_file([26,22]) =  assign_image_file([52,22]);
           assign_image_file([29,21]) =  assign_image_file([21,29]);
           assign_image_file([27,55]) =  assign_image_file([55,27]);
           assign_image_file([30,60]) =  assign_image_file([60,30]);
