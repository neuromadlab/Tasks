...
paradigm_root = pwd;
%paradigm_root = 'C:\Users\TP Experiment\Desktop\Guitart_Masip_task';
...
subj.sessionID = input('Session [1/2]: ','s');
subj.runID = '1';
...
output.trial_order = datasample(trials,length(trials),'Replace',false);
...
% get session specific cues
    ...
    for i_image = 1 : 4 %number of stimuli
        ...
        image_path = [pwd '/images_A/' image_type];
        
      elseif subj.sess == 2
         
        image_path = [pwd '/images_B/' image_type];
...
% payoff1_go = [0.8 0.2 0];
% payoff1_nogo = [0.2 0.8 0];
% payoff2_go = [0 0.8 0.2];
% payoff2_nogo = [0 0.2 0.8];
% payoff3_go = [0.2 0.8 0];
% payoff3_nogo = [0.8 0.2 0];
% payoff4_go = [0 0.2 0.8];
% payoff4_nogo = [0 0.8 0.2];

output.payoff = [0.8 0.2 0;...
                  0.2 0.8 0;...
                  0 0.8 0.2;...
                  0 0.2 0.8;...
                  0.2 0.8 0;...
...
output.time.trial_onset = NaN(trial_num,1);
output.time.cue = NaN(trial_num,1);
output.time.fix1 = NaN(trial_num,1);
output.time.target_circle = NaN(trial_num,1);
output.time.response = NaN(trial_num,10);
output.time.RT = NaN(trial_num,10);
output.time.fix2 = NaN(trial_num,1);
output.time.feedback = NaN(trial_num,1);

output.number_responses = NaN(trial_num,1);
output.key_pressed = NaN(trial_num,10);
output.cue_presented = NaN(trial_num,1);
output.cond_presented = NaN(trial_num,1);
output.circle_correct = NaN(trial_num,1);
output.correct_answer = NaN(trial_num,1);
output.probabilities = NaN(trial_num,1);
output.feedback_cond = NaN(trial_num,1);


%%%%%%%%%%%%%%%%%%%%%% INSTRUCTION %%%%%%%%%%%%%%%%%%%%%%
if strcmp(subj.study, 'TUE002') %skip Intro
...
for i_tr = 1:trial_num
    ...
    %% fractal cue
    cue_num = output.trial_order(i_tr,1);
    ...
    if cue_num == 1
        ...
        Screen('PutImage', screen, fractal1, [setup.screenwidth/3 setup.screenheight/3 setup.screenwidth/3+setup.screenwidth/3 setup.screenheight/3+setup.screenheight/3]);
    ...
    elseif cue_num == 2
        ...
        %Screen('DrawTexture', screen, fractal, [], [setup.screenwidth/3 setup.screenheight/3 setup.screenwidth/3+setup.screenwidth/3 setup.screenheight/3+setup.screenheight/3]);
        Screen('PutImage', screen, fractal2, [setup.screenwidth/3 setup.screenheight/3 setup.screenwidth/3+setup.screenwidth/3 setup.screenheight/3+setup.screenheight/3]);
        output.time.cue(i_tr,1) = Screen('Flip', screen);
        output.time.cue(i_tr,1) = output.time.cue(i_tr,1) - time.paradigm_onset;
        WaitSecs(duration_cue);
    ...
    elseif cue_num == 3
        ...
        %Screen('DrawTexture', screen, fractal, [], [setup.screenwidth/3 setup.screenheight/3 setup.screenwidth/3+setup.screenwidth/3 setup.screenheight/3+setup.screenheight/3]);
        Screen('PutImage', screen, fractal3, [setup.screenwidth/3 setup.screenheight/3 setup.screenwidth/3+setup.screenwidth/3 setup.screenheight/3+setup.screenheight/3]);
        output.time.cue(i_tr,1) = Screen('Flip', screen);
        output.time.cue(i_tr,1) = output.time.cue(i_tr,1) - time.paradigm_onset;
        WaitSecs(duration_cue);
    ...
    elseif cue_num == 4
        ...
        %Screen('DrawTexture', screen, fractal, [], [setup.screenwidth/3 setup.screenheight/3 setup.screenwidth/3+setup.screenwidth/3 setup.screenheight/3+setup.screenheight/3]);
        Screen('PutImage', screen, fractal4, [setup.screenwidth/3 setup.screenheight/3 setup.screenwidth/3+setup.screenwidth/3 setup.screenheight/3+setup.screenheight/3]);
        output.time.cue(i_tr,1) = Screen('Flip', screen);
        output.time.cue(i_tr,1) = output.time.cue(i_tr,1) - time.paradigm_onset;
        WaitSecs(duration_cue);
    ...
    if circle_chance(i_tr,1) == 0 % Check if target circle appears on the left side...
        ...
        output.circle_correct(i_tr,1) = 1;
        ...
        while (duration_target > (GetSecs-time.onset))
            ...
            if c(keys.left) > 0 
                ...
                output.key_pressed(i_tr,key_presses) = keys.left_num;
            ...
            elseif c(keys.right) > 0 
                ...
                output.time.response(i_tr,key_presses) = output.time.response(i_tr,key_presses) - time.paradigm_onset;
                output.number_responses(i_tr,1) = output.number_responses(i_tr,1) +1;
                output.key_pressed(i_tr,key_presses) = keys.right_num;
        ...
        output.circle_correct(i_tr,1) = 2;
    ...
    switch condition_num
        ...
        case 1 % Go to win
            ...
            if (output.circle_correct(i_tr,1) == 1 && output.key_pressed(i_tr,key_presses) == 1) || (output.circle_correct(i_tr,1) == 2 && output.key_pressed(i_tr,key_presses) == 2)
                
                probabilities = output.payoff(1,:);
                output.correct_answer(i_tr,1) = 1;
                output.probabilities(i_tr,1) = 1;
                
            else
                
                probabilities = output.payoff(2,:);
        ...
        case 2 % Go to avoid punishment
            ...
            if (output.circle_correct(i_tr,1) == 1 && output.key_pressed(i_tr,key_presses) == 1) || (output.circle_correct(i_tr,1) == 2 && output.key_pressed(i_tr,key_presses) == 2)
                ...
                probabilities = output.payoff(3,:);
                output.correct_answer(i_tr,1) = 1;
                output.probabilities(i_tr,1) = 3;
                
            else
                
                probabilities = output.payoff(4,:);
                output.correct_answer(i_tr,1) = 0;
        ...
        case 3 % No-Go to win
            ...
            if isnan(output.key_pressed(i_tr,1))
                
                probabilities = output.payoff(6,:);
                output.correct_answer(i_tr,1) = 1;
                output.probabilities(i_tr,1) = 6;
                
            else
                
                probabilities = output.payoff(5,:);
                output.correct_answer(i_tr,1) = 0;
        ...
        case 4 % No-Go to avoid punishment
            ...
            if isnan(output.key_pressed(i_tr,1))
                ...
                probabilities = output.payoff(8,:);
                output.correct_answer(i_tr,1) = 1;
                output.probabilities(i_tr,1) = 8;
                
            else
                
                probabilities = output.payoff(7,:);
                output.correct_answer(i_tr,1) = 0;
...
%% save output
filename = ['GMT_', subj.study, '_', subj.subjectID, '_S', subj.sessionID, '_R', subj.runID];
if do_fmri_flag == 0
