...
for i=1:length(x)
  ...
  r = [ones(n+1,1)];
  theta = theta0 + [x(i)*(0:n)'/n]*2*pi;
  if inside,
    [xtext,ytext] = pol2cart(theta0 + x(i)*pi,0);
  else
    [xtext,ytext] = pol2cart(theta0 + x(i)*pi,1.2);
  end
  [xx,yy] = pol2cart(theta,r/5);
  ...
  theta0 = max(theta);
  h = [h,patch('XData',xx,'YData',yy,'FaceColor','w'),...
               text(xtext,ytext,'!','FontSize',40,'Color','r',...
