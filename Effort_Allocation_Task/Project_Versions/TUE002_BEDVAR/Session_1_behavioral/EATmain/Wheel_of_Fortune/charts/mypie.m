...
for i=1:length(x)
    ...
    if inside,
        [xtext,ytext] = pol2cart(theta0 + x(i)*pi,.75);
    else
        [xtext,ytext] = pol2cart(theta0 + x(i)*pi,1.2);
    end
    
%     %to draw little circle inside pie
%     r_small = 1/5*ones(n+1,1);
%     theta_small = theta0 + x(i)*(0:n)'/n*2*pi;
%     [xx_small, yy_small] = pol2cart(theta_small,r_small);
%     
    ...
    h = [h,patch('XData',xx,'YData',yy,'CData',i*ones(size(xx)), ...
        ...
        text(xtext,ytext,txtlabels{i},'FontSize',22,...
        'HorizontalAlignment','center','parent',cax)];
    hold on;
    
%     %to draw little circle inside pie
%     h = [h,patch('XData',xx,'YData',yy,'CData',i*ones(size(xx)), ...
%         'FaceColor','Flat','parent',cax,'EdgeColor','w'),...
%         patch('XData',xx_small, 'YData',yy_small,'FaceColor','w','EdgeColor','none'),...
%         text(xtext,ytext,txtlabels{i},'FontSize',22,...
%         'HorizontalAlignment','center','parent',cax)];
end
...
n = max(1,ceil(maxpts));
r_small = 1/5*ones(n+1,1);
theta_small = theta0 + (0:n)'/n*2*pi;
[xx_small, yy_small] = pol2cart(theta_small,r_small);

h = [h,patch('XData',xx_small, 'YData',yy_small,'FaceColor','w')];
hold off;
