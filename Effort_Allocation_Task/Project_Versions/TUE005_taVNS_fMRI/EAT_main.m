...
% Information on trial type
subj.runLABEL  = input('Study ID [1 fuer Training / 2 fuer Experiment / 3 fuer Einzeltrial]: ','s');

%study information
subj.study             = 'TUE005';  % Project number
subj.subjectID         = input('Subject ID: ','s');
subj.sessionNR         = input('Session ID [1/2]: ','s');
subj.runID             = '1';

subj.sessionID = subj.sessionNR;

%% Part 1/2: settings and study information
% load settings .mat file
if strcmp(subj.runLABEL, '3') % Check if this is a BID trial
    load(strcat('EATBIDsettings_', subj.study, '_S', subj.sessionNR, '_R', subj.runID, '.mat'))
else
    load(strcat('EATsettings_', subj.study, '_S', subj.sessionNR, '_R', subj.runID, '.mat'))
end
...
if settings.debug == 1
    ...
    FCR_dir = 'D:\home\sektion\AG_Walter\taVNS\FCR\Data\';
end
...
if settings.do_EGG == 1
    ...
    if status == 0
		...
		% Write trigger for signaling start of grEAT 
		io64(LPT_IO_EGG,settings.EGG.port_address,3);
    else
...
% if strcmp(subj.sessionID, subj.sessionNR) == 0
...
if settings.do_VNS == 1 && ~strcmp(subj.runLABEL, 'BID')
...
if strcmp(subj.runLABEL, 'BID')
    subj.difficulty = input('Angebot in Höhe von: ');
    ...
    %input_device.minEffort = 35000;
    %input_device.maxEffort = 12000;
...
if settings.do_fmri == 0
    ...
    if strcmp(subj.runLABEL, 'training') && settings.do_val_cal == 1
        [img.bidding_food_50, img.map, img.alpha]  = imread('snickers_50M.png');
        [img.bidding_food_100, img.map, img.alpha] = imread('snickers_100M.png');
        [img.bidding_food_200, img.map, img.alpha] = imread('snickers_200M.png');
        
        [img.bidding_money_50, img.map, img.alpha]  = imread('money_50M.png');
        [img.bidding_money_100, img.map, img.alpha] = imread('money_100M.png');
        [img.bidding_money_200, img.map, img.alpha] = imread('money_200M.png');
    end
end
...
elseif strcmp(subj.runLABEL, 'BID')
    output.filename = sprintf('BIDEAT_%s_%s_S%s_R%s', subj.study, subj.subjectID, subj.sessionID, subj.runID);
else
...
if  settings.do_gamepad == 1
    ...
    if settings.do_fmri == 1 && strcmp(subj.runLABEL, 'training') && settings.do_val_cal == 1
       save(fullfile('data', [output.filename '.mat']), 'output', 'subj', 'input_device', 'conditions', 'timestamps','MR_timings', 'collectMax', 'collectBid');
       cd(FCR_dir)
