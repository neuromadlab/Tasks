...
if settings.do_fmri == 0
    timings.trial_length        = 2; %change back to 24 after test run
...
output.values_per_trial = []; %every 100 ms for fr, every loop for gr
output.t_payout         = []; %collects all payout across all trials
...
if settings.do_gamepad == 1 % if frEAT
    ...
    %!! evaluate necessity of some of these output structures
    ...
    output.track_button             = []; % stores timestamps of every individual button press
...
%% 16.07 End of trial
    ...
    output.payout_per_trial(3,i_trial) = input_device.incentive;
    output.payout_per_trial(4,i_trial) = input_device.value;
    if isnan(output.payout_per_trial(1,i_trial))
        output.payout_per_trial(5, i_trial) = win_cookies;
    else
        output.payout_per_trial(5, i_trial) = win_coins;
    end
...
if settings.do_VAS == 1
    if settings.VAS.exhaustion == 1
        ...
        output.rating_exhaustion(1,i_trial)          = nan;
    end
    ...
    if settings.VAS.wanting == 1
        ...
        output.rating_wanting(1,i_trial)          = rating;
        ...
        output.rating_wanting(1,i_trial)          = nan;
    end
    ...
    if settings.VAS.happy1 == 1
        ...
        output.rating_happy1_runstart(1,i_trial) = startTime; %Start time of rating
        output.rating_happy1_t_button(i_trial,5) = t_rating_ref; %Time of rating submission
        output.rating_happy1(1,i_trial)          = rating;    
        output.rating_happy1_label{1,i_trial}     = text_freerating;
        output.rating_happy1_subm(1,i_trial)     = 1;
        ...
        output.rating_happy1(1,i_trial)          = nan; 
    end
...
if settings.do_VAS == 1
    if settings.VAS.happy2 == 1
        ...
        output.rating_happy2_runstart(1,i_trial) = startTime; %Start time of rating
        output.rating_happy2_t_button(i_trial,5) = t_rating_ref; %Time of rating submission
        output.rating_happy2(1,i_trial)          = rating;    
        output.rating_happy2_label{1,i_trial}    = text_freerating;
        output.rating_happy2_subm(1,i_trial)     = 1;
        ...
        output.rating_happy2(1,i_trial)          = nan; 
    end
...
output.values_per_trial = [output.values_per_trial, [ones(1,length(effort_vector)) * subj.id; ...                       %ID
                           ones(1,length(effort_vector)) * subj.sess; ...                                               %Sess
                           ones(1,length(effort_vector)) * i_trial;  ...                                                %Trial
                           ones(1,length(effort_vector)) * input_device.maxEffort; ...                                  %Max_Eff
                           ones(1,length(effort_vector)) * input_device.minEffort; ...                                  %Min_Eff
                           t_vector; ...                                                                                %Time_Ref
                           effort_vector; ...                                                                           %Effort
                           rel_Effort; ...                                                                              %relative Effort
                           ones(1,length(effort_vector)) * conditions(i_trial,1); ...                                   %Diff
                           ones(1,length(effort_vector)) * conditions(i_trial,2); ...                                   %Money
                           ones(1,length(effort_vector)) * conditions(i_trial,3); ...                                   %Rew_magn
                           ones(1,length(effort_vector)) * conditions(i_trial,4); ...                                   %Uncertainty
                           ones(1,length(effort_vector)) * output.payout_per_trial(5,i_trial)/conditions(i_trial,3);... %amount of seconds above threshold
                           ones(1,length(effort_vector)) * output.payout_per_trial(5,i_trial)]];                        %Payout (total amount of points)

if settings.do_gamepad == 1
   output.track_button = [output.track_button, [ones(1,length(t_button_ref_vec)) * i_trial; ...
                                                t_button_ref_vec]];                                                      %every time stamp of every button press
end
                       
if settings.do_VAS == 1
    output.VAS_per_trial = [output.VAS_per_trial, [i_trial; ...
                                                   output.rating_exhaustion(i_trial); ...  %exhaustion rating
                                                   output.rating_wanting(i_trial); ...     %wanting rating
                                                   output.rating_happy1(i_trial); ...      %happiness rating 1
                                                   output.rating_happy2(i_trial)]];        %happiness rating 2
end

if settings.do_WOF == 1 && ~strcmp(subj.runLABEL, 'training') && ...
           (i_trial == timings.PANAS_trials(count_PANAS) || i_trial == 1)
    
    output.PANAS.values = [output.PANAS.values, [subj.id ; ...                                      %Subj_ID
                                               i_trial ;  ...                                       %Trial_ID
                                               output.PANAS.rating_active(i_trial); ...             %VAS Rating active
                                               output.PANAS.rating_interested(i_trial); ...         %VAS Rating interested
                                               output.PANAS.rating_excited(i_trial); ...            %VAS Rating excited
                                               output.PANAS.rating_strong(i_trial); ...             %VAS Rating strong
                                               output.PANAS.rating_inspired(i_trial); ...           %VAS Rating inspired
                                               output.PANAS.rating_proud(i_trial);  ...             %VAS Rating proud
                                               output.PANAS.rating_enthusiastic(i_trial); ...       %VAS Rating enthusiastic
                                               output.PANAS.rating_alert(i_trial);  ...             %VAS Rating alert
                                               output.PANAS.rating_determined(i_trial); ...         %VAS Rating determined
                                               output.PANAS.rating_attentive(i_trial); ...          %VAS Rating attentive
                                               output.PANAS.rating_distressed(i_trial); ...         %VAS Rating distressed
                                               output.PANAS.rating_upset(i_trial); ...              %VAS Rating upset
                                               output.PANAS.rating_scared(i_trial); ...             %VAS Rating scared
                                               output.PANAS.rating_hostile(i_trial); ...            %VAS Rating hostile
                                               output.PANAS.rating_irritible(i_trial); ...          %VAS Rating irritible
                                               output.PANAS.rating_ashamed(i_trial); ...            %VAS Rating ashamed
                                               output.PANAS.rating_nervous(i_trial); ...            %VAS Rating nervous
                                               output.PANAS.rating_jittery(i_trial); ...            %VAS Rating jittery
                                               output.PANAS.rating_afraid(i_trial)]];               %VAS Rating afraid
...
if strcmp(subj.runLABEL, 'training')     
    if settings.do_gamepad == 0
        ...
        input_device.maxEffort      = min(input_device.maxEffort, min(collectMax.maxEffort));
    elseif settings.do_gamepad == 1
...
% Prepare feedback
% Compute win
output.win.sum_coins    = floor(nansum(output.payout_per_trial(1,3:end)));
output.win.sum_cookies  = floor(nansum(output.payout_per_trial(2,3:end)));
...
output.data_mat = output.values_per_trial';
