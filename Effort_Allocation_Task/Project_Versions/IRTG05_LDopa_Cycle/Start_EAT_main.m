...
subj.subjectID = input('Subject ID [1-100]: ','s');
validInput = false;
if strcmp(subj.study(1:4),'IRTG')
while ~validInput
    subj.sessionID = input('Session ID [S0/S1/S2/C1]: ','s');
    if strcmp(subj.sessionID,'S0') || strcmp(subj.sessionID,'S1') || strcmp(subj.sessionID,'S2') || strcmp(subj.sessionID,'C1')
        validInput = true;
    else
        warning('Not a valid answer! Please try again.');
    end
end
	subj.subjectID = num2str(str2double(subj.study(end-1:end))*1000 + str2double(subj.subjectID));
else
	subj.sessionID = input('Session ID: ','s');
	subj.sessionID = ['S',subj.sessionID];
end
...
subj.sess       = str2double(subj.sessionID(2));
...
if settings.do_gamepad == 0
    ...
    maxeffort_searchname = [[pwd filesep 'data' filesep 'TrainEAT_' ...
                            subj.study '_'  subj.subjectID '_' subj.sessionID] '*'];
...
if  linux 
    ...
    if strcmp(subj.runLABEL, 'training')
        ...
        cond_filename      = sprintf('%s/conditions/EATTrain_cond_%s_%s_%s_R1.mat', ...
    ...
    elseif strcmp(subj.runLABEL, 'EAT')
        ...
        cond_filename      = sprintf('%s/conditions/EATExp_cond_%s_%s_%s_R1.mat', ...
                                pwd, subj.study, subj.subjectID, subj.sessionID);
...
%%% 12.2: grip force    
    ...
    clckforce           = input_device.minEffort - 0.75* ...
...
if linux || mac
    ...
    output.filename = sprintf('%s/backup/EAT%s_%s_%s_%s_R%s_temp', ...
...
if strcmp(subj.runLABEL, 'training') 
    ...
    if settings.do_gamepad == 0
        input_device.maxEffort      = min(input_device.maxEffort, prctile(effort_vals,5));
    elseif settings.do_gamepad == 1
        if prctile(effort_vals,95) > (1.15*input_device.maxEffort)
            input_device.maxEffort  = 1.15*input_device.maxEffort;
        else
            input_device.maxEffort  = max(input_device.maxEffort, prctile(effort_vals,95));
        end
        input_device.maxEffort      = max(input_device.maxEffort, prctile(effort_vals,95));
...
% Prepare feedback
% Compute win
output.win.sum_coins    = floor(nansum(output.win.payout_per_trial(1,1:end)));
output.win.sum_cookies  = floor(nansum(output.win.payout_per_trial(2,1:end)));
...
%IRTG06 specific
muesli_fix = 300;
if strcmp(subj.study,'IRTG06')
    if subj.sess > 0
        output.win.snack_kcal       = output.win.kcal - muesli_fix; %Snack kcal leftover after muesli
    else
        output.win.snack_kcal       = output.win.kcal;
    end
end
...
elseif strcmp(subj.runLABEL, 'EAT')
   ...
   if settings.do_fmri == 1
        if settings.lang_de == 1
           text = [text, '\n Bitte bleiben Sie still liegen. Die Messungen gehen gleich weiter.'];
       else
           text = [text, '\n Please keep lying still the measurements will continue soon.'];
       end
...
if strcmp(subj.runLABEL, 'training')
    output.filename = sprintf('TrainEAT_%s_%s_%s_R%s', subj.study, subj.subjectID, subj.sessionID, subj.runID);
else
    output.filename = sprintf('ExpEAT_%s_%s_%s_R%s', subj.study, subj.subjectID, subj.sessionID, subj.runID);
end
...
if strcmp(subj.study,'IRTG06') && subj.sess > 0
    sprintf(['Calories for Muesli = ' num2str(muesli_fix)])
end
