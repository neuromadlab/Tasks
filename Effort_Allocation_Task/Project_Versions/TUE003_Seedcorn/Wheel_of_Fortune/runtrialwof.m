...
winloss = output.wof.win_amnt(TempPieIndex,:);
...
if strcmp(settings.lang_de,'1')
    DrawFormattedText(w,'Klicken Sie die rechte Maustaste, um das Gluecksrad drehen zu lassen.', 'center', wdh/10, color.black);
elseif strcmp(settings.lang_de,'2')
    DrawFormattedText(w,'Click the right mouse button to spin the Wheel of Fortune.', 'center', wdh/10, color.black);
end
t0              = Screen('Flip',w);
...
elseif strcmp(settings.lang_de,'2')
    Screen('DrawText',w,'Spinning',setup.xCen,wdh/10,color.red);
end
Screen('Flip',w);
...
elseif strcmp(settings.lang_de,'2')
    Screen('TextSize',w,txtsize_for_header);
    Screen('DrawText',w,'Result',setup.xCen,wdh/10,color.red);
    formatSpec = '%i %s';
    DrawFormattedText(w, sprintf(formatSpec, abwinloss, text),...
        'center',setup.yCen,color.black,60,[],[],1.3);
    Screen('Flip',w);
