...
%===========================================
function Create_Cond_V2(Study_ID,nr_ids,Difficulty,nr_session,i_run,nr_trials,food)
...
for i_s = nr_session(1):nr_session(2)
    ...
    for i_id = start_range:id_range
        ...
        output.filename = sprintf('../conditions/EATTrain_cond_%s_%06d_%s_R%s', subj.studyID, i_id,subj.study_part_ID, subj.studyRun);
    ...
    %for i_run = nr_runs(1):nr_runs(2)  %habe ich auskommentiert, Luisa K.
        for i_id = start_range:id_range 
            ...
            %num2str(i_run);Luisa K.
            conditionstable = array2table(conditions, 'VariableNames', {'Difficulty', 'Money', 'Rew_magn', 'Uncertainty'});
            output.filename = sprintf('../conditions/EATExp_cond_%s_%06d_%s_R%s', subj.studyID, i_id, subj.study_part_ID, subj.studyRun); %Luisa K.
            save([output.filename '.mat'], 'conditions', 'Value_labels', 'conditionstable')
