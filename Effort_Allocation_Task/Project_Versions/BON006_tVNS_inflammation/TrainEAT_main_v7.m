...
for i_collectMax = 1:settings.train_trials
    ...
    while (10  > (GetSecs - t_collectMax_onset))
        ...
        if settings.do_gamepad == 0
            if ForceMatR > initrestforce
                %                  Boundary_yposition = BarBound2Scale*ForceMat + UpperBoundBar + initmaxforce * BarBound2Scale;
                Boundary_yposition = LowerBoundBar - BarBound2Scale*(ForceMatR - initrestforce);
            else
    ...
    collectMax.filename = sprintf('%s\\data\\effort_%s_%s_S%s_R%s_temp', pwd, subj.study, subj.subjectID, subj.sessionID, subj.runID);
