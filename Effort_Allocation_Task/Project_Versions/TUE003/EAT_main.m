...
% Integration WOF: Emily Corwin-Renner, updated by Wy Ming Lin
% smoothness fix: Wy Ming Lin
%
...
subj.sessionID         = '1'; % 0 means settings are the same for both sessions
subj.runID             = '1';

%% Part 1/2: settings and study information 
load(strcat('EATsettings_', subj.study, '_S', subj.sessionID, '_R', subj.runID, '.mat'))
...
% Path to data folder
curdir  = pwd;
% idcs    = strfind(curdir,filesep);
% datadir  = curdir(1:idcs(end)-1);
datadir = curdir;
...
subj.runLABEL  = input('Study ID [1 for Training / 2 for Experiment / 3 for Einzeltrial]: ','s');
subj.subjectID = input('Subject ID: ','s');
settings.lang_de = input('Language [German = 1, English = 2]: ','s');
...
% if strcmp(subj.sessionID, subj.sessionNR) == 0 && subj.sessionNR ~= 0
%     error('Error: session ID indicated does not match hardcoded session number. Please review line 21 or input.')
% end
...
elseif strcmp(subj.runLABEL, '2')
    subj.runLABEL = 'EAT'; 
elseif strcmp(subj.runLABEL, '3')
    subj.runLABEL = 'BID';
end
...
% If single trial
if strcmp(subj.runLABEL, 'BID')
    subj.difficulty = input('Angebot in H�he von: ');
    input_device.minEffort = 15000;
    input_device.maxEffort = -15000;
end
...
maxeffort_searchname = [[datadir filesep 'data' filesep 'TrainEAT_' ...
            subj.study '_'  subj.subjectID '_S' num2str(settings.load_max_sess)] '*'];
...
if  linux || mac
    ...
    elseif strcmp(subj.runLABEL, 'EAT')
        ...
        maxeffort_filename = sprintf('%s/data/%s', datadir, maxeffort_searchdir.name);
...
elseif strcmp (subj.runLABEL, 'training')
    load(cond_filename);
    load(bid_filename)
elseif strcmp(subj.runLABEL, 'BID')
    conditions = [subj.difficulty, 1, 1, 0];
end
...
% TempPieIndex = wof trial number
% Only extra script needed to run the WoF trials is 'runtrialwof.m'
% 'Spinner.mat' also needed for proper functioning of the WoF trials
...
if settings.do_WOF == 1
    ...
    % shuffle win variations (0, -1, +1) and determine final outcomes for
    % WoF trials such that no wins of zero ever occur
    while 1
        variations_shuffled = settings.wof.win_variation(randperm(length(settings.wof.win_variation)));
        output.wof.win_amnt = settings.wof.win_sequence + variations_shuffled;
        if ~any(output.wof.win_amnt == 0)
            break
        end
    end
    ...
    %displayed when the lottery is played out
...
if settings.do_gamepad == 1 % if frEAT
    ...
    forget_fact         = 0.6; % A forgetting factor of 0.9 gives more weight to the older data than does a forgetting factor of 0.1. 
    i_phantom           = 1;
    prev_weight_fact    = 0;
    prev_movingAvrg     = 0;
    current_input       = 0; 
...
%     EffortHigh = input_device.maxEffort * 0.95;
    EffortLow  = input_device.maxEffort * 0.55;
    EffortHigh = input_device.maxEffort * 0.90;
...
if strcmp(subj.runLABEL, 'training')
    ...
    if settings.do_fmri == 0
        GetClicks(setup.screenNum);
        if settings.do_VNS == 1
        [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, instr.intro_tvns, 'center', Text.height, color.black, 60, flp_flg_hrz, flp_flg_vrt, 1.2);
        [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, instr.text_Cont, 'center', Text.height_cont, color.black, 50, flp_flg_hrz, flp_flg_vrt, 1.2);
        Screen('Flip',w);
        GetClicks(setup.screenNum);
        end
        if settings.do_uncertain == 1
           [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, instr.intro_uncer, 'center', Text.height, color.black, 60, flp_flg_hrz, flp_flg_vrt, 1.2);
           [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, instr.text_Cont, 'center', Text.height_cont, color.black, 50, flp_flg_hrz, flp_flg_vrt, 1.2);
           Screen('Flip',w);
           GetClicks(setup.screenNum);
        end
...
if settings.do_fmri == 0
    ...
    % Set generic value labels if val val is done
    if strcmp(subj.runLABEL, 'training') && settings.do_val_cal == 1
        input_device.value_money = 32;
        input_device.value_food = 64;
    end
    ...
    elseif strcmp(settings.lang_de,'2')
        ...
        instr.reward_nr4         = ['The points will be converted with the following rates:  '...
                                    ...
                                    '\n\nAfter the task, you can trade the Money points in for money'...
                                    'and the Snack points for snacks.'];
...
elseif ~strcmp(subj.runLABEL, 'training')
   ...
   instr.bidphase = 'Start mit Mouseklick';
   [pos.text.x,pos.text.y,post.text.bbox] = DrawFormattedText(w, instr.bidphase,'center', Text.height, color.black, 60, flp_flg_hrz, flp_flg_vrt, 1.2);
   [pos.text.x,pos.text.y,post.text.bbox] = DrawFormattedText(w, instr.text_Cont, 'center', Text.height_cont, color.black, 50, flp_flg_hrz, flp_flg_vrt, 1.2);
   Screen('Flip',w);
   GetClicks(setup.screenNum);
end
...
for i_trial = 1:length(conditions) %condition file determines repetitions
    ...
    if settings.do_EGG == 1 && ~strcmp(subj.runLABEL, 'training')
        if input_device.incentive == 1
            ...
            io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.cue_money);
        elseif input_device.incentive == 0
            % Write trigger for EGG - cue for food reward
            io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.cue_food);
        end
    end
    ...
    % Trigger EGG
    if settings.do_EGG == 1 && ~strcmp(subj.runLABEL, 'training')
        % Write trigger for EGG - cue presentation end
        io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.cue_end);
    end
    ...
    if settings.do_EGG == 1 && ~strcmp(subj.runLABEL, 'training')
        ...
        io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.work_block); 
    end
    ...
    %% 16.06.1 Draw graphical display
      ...
      if Ball.position(1,4) < Threshold.yposition  
          ...
          if (flag == 0) % Mark "crossing the threshold"
             ...
             % Trigger EGG
             if settings.do_EGG == 1 && ~strcmp(subj.runLABEL, 'training')
                % Write trigger for EGG - win phase start
                io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.win);
             end
        ...
        % -> change color, stop increasing score 
             ...
             if flag == 1 % Mark "crossing the threshold"
                 ...
                 % Trigger EGG
                 if settings.do_EGG == 1 && ~strcmp(subj.runLABEL, 'training')
                    % Write trigger for EGG - rest phase start
                    io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.rest);
                 end
...
%% Part 16.06.5: frEAT
    if settings.do_gamepad == 1
        ...
        for buffer_i = 2:50 %buffer_size
            ...
            % maintaining draw_frequency from last button press
...
%% 16.07 End of trial
    ...
    % Trigger EGG
    if settings.do_EGG == 1 && ~strcmp(subj.runLABEL, 'training')
       % Write trigger for EGG - end work block
       io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.work_block_end);
    end
...
if settings.do_EGG == 1 && ~strcmp(subj.runLABEL, 'training')
    if conditions(i_trial,4) == 1
        ...
        io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.feedback_uncertain);
    elseif conditions(i_trial,4) == 0
        % Write trigger for EGG - feedback certain
        io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.feedback_certain);
    end
end
...
% Trigger EGG
if settings.do_EGG == 1 && ~strcmp(subj.runLABEL, 'training')
    % Write trigger for EGG - feedback end
    io64(LPT_IO_EGG,settings.EGG.port_address, settings.EGG.trigger.feedback_end);
end
...
if linux || mac
    ...
    output.filename = sprintf('%s/backup/EAT%s_%s_%s_S%s_R%S_temp', ...
                                datadir, subj.runLABEL, subj.study, subj.subjectID, subj.sessionID, subj.runID);    
elseif windows
...
if strcmp(subj.runLABEL, 'training') 
    ...
    elseif settings.do_gamepad == 1
        if max(effort_vals) > (1.15*input_device.maxEffort)
...
if settings.do_WOF == 1
    % End WOF or familiarization WOF
    if strcmp(subj.runLABEL, 'training') 
        TempPieIndex = 1;
        runtrialwof
    else
...
if strcmp(subj.runLABEL, 'training') 
    if strcmp(settings.lang_de,'1')
        text = ['Das war die Uebung zur eigentlichen Experiment.'...
    ...
    elseif strcmp(settings.lang_de,'2')
        text = ['That was the practice for the actual experiment.'...
            '\n\nIf this were the real game, you would have won ' num2str(output.win.sum_coins) ' Money points. '...
            '\n This corresponds to ' num2str(output.win.money) ' Euros. '...
            '\n\nIn this were the real game, you would have won ' num2str(output.win.sum_cookies) ' Snack points. '...
...
elseif strcmp(subj.runLABEL, 'EAT')
   ...
   if settings.do_fmri == 1
       ...
       elseif strcmp(settings.lang_de,'2')
            text = [text, '\n Please continue lying down still until we come in the room to get you.'];
       end
   end
else 
   if strcmp(settings.lang_de,'1')
       text = ['\n\n Sie haben sich ' num2str(output.win.payout_per_trial(1,1)) ' Gewinn-Punkte erspielt.'];
   elseif strcmp(settings.lang_de,'2')
       text = ['\n\n You won ' num2str(output.win.payout_per_trial(1,1)) ' points.'];
   end
end
...
[~,~,~] = DrawFormattedText(w, instr.text_Cont, 'center', Text.height_cont, color.black, 50, flp_flg_hrz, flp_flg_vrt, 1.2);
Screen('Flip',w);
GetClicks(setup.screenNum);
...
if strcmp(subj.runLABEL, 'training')
    if settings.do_val_cal == 1
        ...
        % closing screen after the bidding phase
        if strcmp(settings.lang_de,'1')
            text = ['Die Uebungsphase ist nun zu Ende. Wenden Sie bitte an die Versuchsleitung.'];
        elseif strcmp(settings.lang_de,'2')
            text = ['The practice phase has now ended. Please refer to the experimenter.'];
        end
...
output.variable_labels = {'ID', 'Session', 'Trial', 'MaxEff', 'MinEff', 'Time', 'AbsEff', 'RelEff', 'Difficulty', ...
                 'Money', 'RewMag', 'Uncertainty', 'WinSec', 'WinPoint'};
...
if  settings.do_gamepad == 1
   ...
   save(fullfile('backup', [output.filename datestr(now,'_yymmdd_HHMM') '.mat']));
    ...
    save(fullfile('data', [output.filename '.mat'], 'output', 'subj', 'input_device', 'conditions'));
end
...
if ~strcmp(subj.runLABEL, 'BID')
    ...
    sprintf(['Win points = ' num2str(output.win.payout_per_trial(1,1))]);
end
if settings.do_WOF == 1 && ~strcmp(subj.runLABEL, 'training')
