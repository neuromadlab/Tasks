...
if settings.do_gamepad == 1 % if frEAT
    ...
    if training_run == 2
        temp_maxEffort = input_device.maxEffort; % store maxEffort from calibration step earlier
    end
    ...
    elseif mac
        ...
        initrestforce   = 15000; %normal holding force
        initmaxforce    = -15000; %upper limit of GFD   
        clckforce       = -10000;
        axisState       = Gamepad('GetAxis', grip_force_idx, grip_force_axis);
        gripforce_value = axisState;
...
if training_run == 1
    ...
    %         collectMax.minEffort(1,i_collectMax) = nan;  
        ...
        if windows
            collectMax.filename = sprintf('%s\\data\\effort_%s_%s_s%s_temp', datadir, subj.study, subj.subjectID, subj.sessionID);
...
if training_run == 2
    if settings.do_val_cal == 1
        ...
        final_effort = zeros(1, 18);
        random_bidcond = bidcond(randperm(size(bidcond, 1)), :); 
        bidding.food = zeros(2,9);
        f_bidcount = 1;
        bidding.money = zeros(2,9);
        m_bidcount = 1;
        ...
        for calround = 1:length(random_bidcond)
            i_collectMax = i_collectMax + 1;

            % Prepare graphical display with corresponding reward items    
            % load incentive & counter icon
            if random_bidcond(calround, 1) == 1
                incentive = stim.(['bidding_money' num2str(random_bidcond(calround, 2))]);
            elseif random_bidcond(calround, 1) == 0
                incentive = stim.(['bidding_food' num2str(random_bidcond(calround, 2))]);
            end
...
if training_run == 2
    input_device.maxEffort = temp_maxEffort; % store maxEffort from calibration step earlier
end
