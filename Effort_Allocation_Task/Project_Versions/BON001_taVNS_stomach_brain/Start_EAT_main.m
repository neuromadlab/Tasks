...
elseif settings.do_gamepad == 1
    maxeffort_searchname = [[pwd filesep 'data' filesep 'TrainEAT_' ...
                            subj.study '_'  subj.subjectID '_S3'] '*'];
end
      ...
      if settings.do_gamepad == 0
        % Track Ball position and translate into payout
         ...
         if (ForceMatR > restforceR) || (ForceMatL > restforceL)
            ...
            Ball_yposition = min(Ball_ypositionR, Ball_ypositionL);
         else
...
if strcmp(subj.runLABEL, 'training') 
    ...
    if settings.do_gamepad == 0
        ...
        input_device.maxEffortL      = max(input_device.maxEffortL, max(effort_valsL));
        input_device.maxEffortR      = max(input_device.maxEffortR, max(effort_valsR));
    elseif settings.do_gamepad == 1
...
output.variable_labels = {'ID', 'Session', 'Run', 'Trial', 'Maximum Effort', 'Minimum Effort', 'Time',...
    'Absolute Effort', 'Absolute Effort Left', 'Absolute Effort Right', 'Relative Effort', 'Difficulty', ...
