...
subj.studyID = 'TUE008'; %Prefix of tVNS project
subj.study_part_ID = 'S2'; %indicates session
start_range = 000001;
id_range = 000100;
...
%Condition vector -> 8 trials in training changed to 4 trials
% in these vectors, first 2 lines are option 1, last 2 lines are option 2
LowDiff_vectors  = [LowDiff_M_low; LowDiff_F_high; LowDiff_M_high; LowDiff_F_low];
HighDiff_vectors = [HighDiff_M_low; HighDiff_F_high; HighDiff_M_high; HighDiff_F_low];

for i_id = start_range:id_range
    
    conditions = [];
    
    % Random selection of lowdiff and highdiff vectors
    k_LowDiff = randsample([1,3],1);
    LowDiff_vector = LowDiff_vectors(k_LowDiff:k_LowDiff+1, :);
    k_HighDiff = randsample([1,3],1);
    HighDiff_vector = HighDiff_vectors(k_HighDiff:k_HighDiff+1,:);

    %Random order of trials within a vector
    perm_i_LowDiff  = randperm(height(LowDiff_vector));
    perm_i_HighDiff = randperm(height(HighDiff_vector));


    %Create conditions vector, HighDiff and LowDiff alternating
    for k = 1:height(LowDiff_vector)

        l_k = LowDiff_vector(perm_i_LowDiff(k),:);
        h_k = HighDiff_vector(perm_i_HighDiff(k),:);
