...
%study information
subj.study             = 'TUE009';  % Project number
subj.sessionNR         = '2';
subj.runID             = '1'; 
...
settings.do_EGG        = 0;  % will include EGG trigger
settings.debug         = 0;  % input device not required
settings.do_gamepad    = 1;  % (1) for frequency, (0) for grip force
...
settings.do_VNS        = 0;  % stimulation (1) or not (0)
settings.load_max_sess = 1;  % session from which the maxEffort needs to be loaded
settings.do_uncertain  = 1;  % uncertainty used in paradigm (1) or not (0)
...
instr.vas_nr1            = ['Nach jedem Durchgang werden Ihnen nacheinander zwei Fragen angezeigt:'...
                            ...
                            ' Ihre Antwort muessen Sie dann mit einem gruenen A-Taste auf dem Controller bestaetigen.'...
...
%% Save
name_file = strcat('settings/EAT_Settings_', subj.study, '_S', subj.sessionNR, '_R', subj.runID, '.mat');
save(name_file,'breaknumbr','brk','instr','settings','timings')