...
subj.sessionID = input('Session ID [0/1/2/3/4]: ','s');
...
if subj.sessionID =='1'
    load('./settings/EAT_Settings_TUE009_S1.mat')
else
    load('./settings/EAT_Settings_TUE009_S2.mat')
end
...
elseif settings.do_gamepad == 1
    maxeffort_searchname = [[pwd filesep 'data' filesep 'TrainEAT_' ...
                            subj.study '_'  subj.subjectID '_S1'] '*']; %loads max force from session 1 so it is not updated in later trainings!! careful with other session specific settings stored in input_device (e.g., food/money values)
end
...
if strcmp(subj.runLABEL, 'training')
    ...
    [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, instr.text_Cont, 'center', Text.height_cont, color.black, 50, flp_flg_hrz, flp_flg_vrt, 1.2);
    Screen('Flip',w);
    if settings.do_fmri == 0 && subj.sessionID ~='1'
        GetClicks(setup.screenNum);
        ...
        Screen('Flip',w);
        GetClicks(setup.screenNum);
    elseif settings.do_fmri == 0 && subj.sessionID =='1'
        GetClicks(setup.screenNum);
    elseif settings.do_fmri == 1
        WaitSecs(3);        
        while gripforce_value > clckforce
...
if settings.do_fmri == 0
    ...
    %source for values changed in TUE009 if you use individually calibrated
    %values for food vs money use values stored in input_device instead. 
...
% TUE009 specific Musli in S1 only Snacks in S2-5:
if subj.sessionID =='1'
...
if snack_kcal >99
    output.win.snacks = floor(snack_kcal/100);
elseif snack_kcal >= 50
    output.win.snacks = 0.5;
elseif snack_kcal >= 25
    output.win.snacks = 0.25;
else 
    output.win.snacks = 0;
end
...
elseif strcmp(subj.runLABEL, 'EAT')
    if settings.lang_de == 1
         text = ['Das Spiel ist nun zu Ende.'...
            ...
            '\n Das entspricht ' num2str(output.win.snacks) ' Snacks.'];
    else
        text = ['The game is now complete.'...
            ...
            '\n This corresponds to ' num2str(output.win.snacks) ' Snacks'];
    end
   if settings.do_fmri == 1
       if settings.lang_de == 1
...
sprintf(['Nr. of snacks = ' num2str(output.win.snacks)])
if subj.sessionID =='1'
