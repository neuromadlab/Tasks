...
% 
%author: Sophie Mueller, adapted by Kristin Kaduk
%
...
%% Enter subject data
subj.study_id = 'TUE012';
subj.id = input('Subject ID (6 digits!)\n','s');
subj.session = input('Session (1/2/3)\n');
subj.language = input('Language (d/e): \n','s'); %enter de (-> German) or en (-> English)

if strcmpi(subj.language,'d')
    lang_de = 1;
    disp('Run in German.')
else
    lang_de = 0;
...
SymptomSilhouette       = settings.pic;
...
texture.Symptom     = Screen('MakeTexture', scr, SymptomSilhouette); 
...
% Instructions screen
if lang_de == 1
    text = ['In der folgenden Aufgabe geht es um Ihre persoenlichen Wahrnehmungen der koerperlichen Symptome Ihrer Erkrankung mit Long COVID. Bitte waehlen Sie alle Bereiche auf der Koerpersilhouette aus, die fuer die jeweilige Frage zutreffen. Die Silhouette bezieht sich immer auf die Koerpervorderseite.'];
else
    ...
    %         DrawFormattedText(scr, text_Button, setup.ScrCenter(1)*1.5-15 , setup.ScrCenter(2)*1.5+10, [1 1 1], 60, [], [], 1.2);
        ...
        if x >= setup.ScrCenter(1)*1.5-50 && x <= setup.ScrCenter(1)*1.5+50 && y >= setup.ScrCenter(2)*1.5-50 && y <= setup.ScrCenter(2)*1.5+50
            if isnan(settings.flag_left)
                if lang_de == 1
                    text = ['Sie muessen eine Seite markieren.'];
                else
...
if settings.flag_left == 1 %if flag_left
    SymptomSilhouette = flip(SymptomSilhouette,2);
...
%% Localication of symptoms  Silhouette
if lang_de == 1
    text = ['Wo nehmen Sie in den letzten vier Wochen koerperliche Symptome im Vergleich zur Zeit vor Ihrer Erkrankung mit Long Covid wahr? Bitte markieren Sie auf dieser Silhouette alle Koerperbereiche.'];
else
    text = ['Where have you noticed physical symptoms in the last four weeks compared to the time before your Long Covid illness? Please mark on the silhouette all of the areas on the body. ']; %english translation
end
...
% DrawFormattedText(scr, text_Button, setup.ScrCenter(1)*1.5-15 , setup.ScrCenter(2)*1.5+10, [1 1 1], 60, [], [], 1.2);
Screen('DrawTexture', scr, texture.Continue, [],[setup.ScrCenter(1)*1.5-50 setup.ScrCenter(2)*1.5-50 setup.ScrCenter(1)*1.5+50 setup.ScrCenter(2)*1.5+50])
Screen('DrawTexture', scr, texture.Symptom,[],[setup.ScrCenter(1)-106 setup.ScrCenter(2)-310 setup.ScrCenter(1)+106 setup.ScrCenter(2)+310])
Screen('Flip',scr)
WaitSecs(0.5)
...
%         DrawFormattedText(scr, text_Button, setup.ScrCenter(1)*1.5-15 , setup.ScrCenter(2)*1.5+10, [1 1 1], 60, [], [], 1.2);
    ...
    frame.R = SymptomSilhouette(y_pic-settings.brush_size+y_lower:y_pic+settings.brush_size-y_upper,x_pic-settings.brush_size+x_lower:x_pic+settings.brush_size-x_upper,1);
    frame.G = SymptomSilhouette(y_pic-settings.brush_size+y_lower:y_pic+settings.brush_size-y_upper,x_pic-settings.brush_size+x_lower:x_pic+settings.brush_size-x_upper,2);
    frame.B = SymptomSilhouette(y_pic-settings.brush_size+y_lower:y_pic+settings.brush_size-y_upper,x_pic-settings.brush_size+x_lower:x_pic+settings.brush_size-x_upper,3);
    ...
    frame.brush_log = i_brush(i_brush ~=0); %(i_brush~=0); %gaussian color increase parameter for circular shape
...
colorB = 245; 
       if button(1) == 1 %left mouse button = more activation (increasing color intensity), color pattern: white -> yellow -> orange -> red
        for i = 1:length(frame.RGB_log) % run through each pixel
            ...
            if i_RGB(1,1) > colorR && i_RGB(1,2) > colorG && i_RGB(1,3)> colorB %i_RGB(1,1:3)  > colorDark %
                frame.RGB_log(i,1) = i_RGB(1)*13.4*settings.colorSpeed; %-frame.brush_log(i)*13.4*settings.colorSpeed;
                frame.RGB_log(i,2) = i_RGB(2)*13.4*settings.colorSpeed; %-frame.brush_log(i)*13.4*settings.colorSpeed;
                frame.RGB_log(i,3) = i_RGB(3)*13.4*settings.colorSpeed; %-frame.brush_log(i)*13.4*settings.colorSpeed;
                if frame.RGB_log(i,1) < colorR
                    frame.RGB_log(i,1) = colorR;
                end
                if frame.RGB_log(i,2) < colorG
                    frame.RGB_log(i,2) = colorG;
                end
                if frame.RGB_log(i,3) < colorB
                    frame.RGB_log(i,3) = colorB;
                end
%               if frame.RGB_log(i,3) < colorDark ||  frame.RGB_log(i,2) < colorDark
%                     frame.RGB_log(i,1) = colorDark;
%                     frame.RGB_log(i,2) = colorDark;
%                     frame.RGB_log(i,3) = colorDark;
%               end   
        ...
        SymptomCourse(ActionCount,:) = [x_pic, y_pic, 1]; %store performed action
     ...
     elseif button(3) == 1 %right mouse button = less activation (decreasing color intensity) red -> orange -> yellow -> white
         for i = 1:length(frame.RGB_log) % run for each pixel
             i_RGB = frame.RGB_log(i,:);
             %if %i_RGB(1,1:3)  > colorDark-10 %& i_RGB(1,3) >= 51 % check if the pixel is white
             if  i_RGB(1,1) >= colorR && i_RGB(1,2) >= colorG && i_RGB(1,3)>= colorB %i_RGB(1,1:3)  > colorDark %
                 frame.RGB_log(i,1) = i_RGB(1)+frame.brush_log(i)*20.4*settings.colorSpeed;
                 frame.RGB_log(i,2) = i_RGB(2)+frame.brush_log(i)*20.4*settings.colorSpeed;
    ...
    SymptomSilhouette(y_pic-settings.brush_size+y_lower:y_pic+settings.brush_size-y_upper,x_pic-settings.brush_size+x_lower:x_pic+settings.brush_size-x_upper,1) = frame.R;
    SymptomSilhouette(y_pic-settings.brush_size+y_lower:y_pic+settings.brush_size-y_upper,x_pic-settings.brush_size+x_lower:x_pic+settings.brush_size-x_upper,2)= frame.G;
    SymptomSilhouette(y_pic-settings.brush_size+y_lower:y_pic+settings.brush_size-y_upper,x_pic-settings.brush_size+x_lower:x_pic+settings.brush_size-x_upper,3)= frame.B;
...
save([pwd '\Data\silhouette_' subj.study_id '_' num2str(subj.id) '_S' num2str(subj.session)], 'SymptomSilhouette', 'ActivationSilhouette', 'DeactivationSilhouette', 'SymptomCourse','ActivationCourse', 'DeactivationCourse', 'subj','settings')
save([pwd '\Backup\silhouette_' subj.study_id '_' num2str(subj.id) '_S' num2str(subj.session)], 'SymptomSilhouette', 'ActivationSilhouette', 'DeactivationSilhouette','SymptomCourse', 'ActivationCourse', 'DeactivationCourse', 'subj','settings')

%% Activation Silhouette
if lang_de == 1
    text = ['Wo haben Sie staerkere koerperliche Empfindungen (wie z.B. Schmerz) im Vergleich zur Zeit vor Ihrer Erkrankung mit LongCovid? Bitte markieren Sie auf dieser Silhouette alle Koerperbereiche.'];
else
    text = ['Where did you experience stronger bolidy senstaions (such as pain) in comparision to the time before your LongCovid diagnoses? Please mark on the silhouette all of the areas on the body.']; %english translation
end
...
ActionCount = 1; %reset ActionCount for activation part
...
%save current results
save([pwd '\Data\silhouette_' subj.study_id '_' num2str(subj.id) '_S' num2str(subj.session)], 'SymptomSilhouette', 'ActivationSilhouette', 'DeactivationSilhouette', 'SymptomCourse', 'ActivationCourse', 'DeactivationCourse', 'subj','settings')
save([pwd '\Backup\silhouette_' subj.study_id '_' num2str(subj.id) '_S' num2str(subj.session)], 'SymptomSilhouette', 'DeactivationSilhouette','SymptomCourse',  'ActivationCourse', 'DeactivationCourse', 'subj','settings')

%% Deactivation silhouette
if lang_de == 1
    text = ['Wo haben Sie schwaechere koerperliche Empfindungen (wie eine reduzierte Wahrnehmung der Koerperregionen) im Vergleich zur Zeit vor Ihrer Erkrankung mit Long Covid? Bitte markieren Sie alle Koerperbereiche.'];
else
    text = ['Where did you experience weaker bodily sensation in comparison to the time before your Long COVID diagnoses? Please mark on the silhouette all of the areas on the body.']; %english translation
end
Screen('TextSize',scr,32);
Screen('TextFont',scr,'Arial');
DrawFormattedText(scr, text, 'center', setup.ScrCenter(2)-450, [255 255 255], 60, [], [], 1.2);
