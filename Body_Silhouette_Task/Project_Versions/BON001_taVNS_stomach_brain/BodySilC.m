...
%modified by Annia von Gehlen (09/11/2023) for BON001, changing
%instructions & number of sessions/conditions 
%==========================================================================
...
%% Enter subject data
subj.study_id = 'BON001';
subj.id = input('Subject ID [6 digits!]: ','s');
subj.session = input('Session [3, 4, 5, or 6]: ');
subj.language = input('Language [1 = German/2 = English]: ');
...
% Instructions screen
if lang_de == 1
    text = ['In der folgenden Aufgabe geht es um Ihre persoenlichen Wahrnehmungen an Ihrem Koerper. Bitte waehlen Sie alle Bereiche auf der Koerpersilhouette aus, die fuer die jeweilige Frage zutreffen. Die Silhouette bezieht sich immer auf die Koerpervorderseite.'];
else
    text = ['The following task will test how you perceive your body. Please select all areas on the silhouette that answer or apply to each question. The silhouette always corresponds to the front side of the body.']; %english translation
end
...
if lang_de == 1
    text = ['Bitte markieren Sie auf dieser Silhouette alle Koerperbereiche, in welchen Sie stärkere Aktivierung als gewöhnlich spüren (z.B. Schmerz, Anspannung, Glücksgefühle, Antrieb).'];
else
    text = ['Please mark on the silhouette all of the areas on the body where you feel stronger activation than usual (e.g. pain, tension, elation, drive).']; %english translation
end
Screen('TextSize',scr,32);
Screen('TextFont',scr,'Arial');
...
save([pwd '\Backup\silhouette_' subj.study_id '_' num2str(subj.id) '_S' num2str(subj.session) datestr(now,'_yymmdd_HHMM')], 'ActivationSilhouette', 'DeactivationSilhouette', 'ActivationCourse', 'DeactivationCourse', 'subj','settings')

%% Deactivation silhouette
if lang_de == 1
    text = ['Bitte markieren Sie auf dieser Silhouette alle Koerperbereiche, in welchen Sie schwächere Aktivierung als gewöhnlich spüren (z.B. Entspannung, ruhiger Herzschlag, Taubheit).'];
else
    text = ['Please mark on the silhouette all of the areas on the body where you feel less activation than usual (e.g. relaxation, slow heartbeat, numbness).']; %english translation
end
Screen('TextSize',scr,32);
Screen('TextFont',scr,'Arial');
DrawFormattedText(scr, text, 'center', setup.ScrCenter(2)-450, [255 255 255], 60, [], [], 1.2);
