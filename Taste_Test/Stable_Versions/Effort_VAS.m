...
jitter_factor = [1 -1];
jitter_factor = jitter_factor(randi(2,1));
...
loop_start = GetSecs;
if control_joystick == 1
    while (Joystick.X < x_mid * 0.85) || (Joystick.X > x_mid * 1.15) || (Joystick.Y < y_mid * 0.85) || (Joystick.Y > y_mid * 1.15) ||any(Joystick.Button)
...
if settings.lang_de == 1
    ...
    elseif strcmp(scale_type,'taste')       
        if strcmp(lang.question.(language){i_rating,1},'Umami')
...
if  strcmp(trial.question,'wanting')
    if settings.lang_de == 1
        text_question = ['Wie sehr wollen Sie den Snack in diesem Durchgang erhalten?'];
    else
        text_question = ['How much do you want to receive the snack in this trial?']; %english translation
    end
elseif  strcmp(trial.question, 'liking')
    if settings.lang_de == 1
        text_question = ['Bitte bewerten Sie den Snack im Vergleich zu allen bisher in Ihrem Leben erfahrenen Empfindungen.'];
        ...
        text_question = ['Please rate the snack in comparison with all of the sensations you have experienced in your life.']; %english translation
...
if strcmp(trial.question, 'liking') % vertical scale
    ...
    DrawFormattedText(rating_scr, text_question, position_task_x - 150, 'center', color_scale_anchors,40,[],[],2);
    ...
    Y = round(Scale_offset + Scale_height * 0.50) + jitter_slider(count_jitter_slider)*(Scale_height/Scale_length)*jitter_factor; %Fix y coordinate
    count_jitter_slider = count_jitter_slider +1;
    Slider_y_pos = Y;
    
    if control_joystick == 1
        scale_joy_x = setup.ScrWidth*0.7/JoystickSpecification.Max;
        scale_joy_y = setup.ScrHeight*0.9/JoystickSpecification.Max;
    else
        SetMouse(X,Y);
    end
    ...
    if control_joystick == 1
        ...
        proj_y = offset_y + round(Joystick.Y * scale_joy_y);
    else
    ...
    while (GetSecs - t_scale_trigger) < max_dur_rating && flag_resp == 0 %mousebuttons(1)~=1 
        ...
        if (GetSecs > nextTime) %Sample every 0.01 seconds
            ...
            if flag_resp==1 %Terminate and record rating on left mouseclick
                ...
                rating = 100-((Slider_y_pos - Scale_offset)/Scale_height)*Scale_length; %rescaling of scale_height independent of screen resolution [0-100]
                rating_subm = 1;
    ...
    if flag_resp==0
            ...
            rating = 100-((Slider_y_pos - Scale_offset)/Scale_height)*Scale_length;
            rating_subm = 0;
            t_rating_ref = GetSecs - startTime;
...
elseif strcmp(trial.question,'taste')
    ...
    X = round(setup.ScrWidth/2);
    Y = round(Scale_offset + Scale_height/2) + jitter_slider(count_jitter_slider)*(Scale_height/Scale_length)*jitter_factor;%Fix y coordinate
    count_jitter_slider = count_jitter_slider +1; 
    Slider_y_pos = Y;
...
%     Screen('DrawTexture',rating_scr,texture_i,[],[(setup.ScrWidth/2-Scale_width/3) (Scale_offset+setup.ScrHeight/2-500) (setup.ScrWidth/2+Scale_width/3) (Scale_offset+setup.ScrHeight/2-50)]);
 ...
 %--- Start display for trial---
    ...
    X = round(setup.ScrWidth/2) + jitter_slider(count_jitter_slider)*(Scale_width/Scale_length)*jitter_factor;
    count_jitter_slider = count_jitter_slider +1;
           ...
           if control_joystick == 1
            ...
            elseif control_joystick == 0 && mousebuttons(1) == 1
                ...
                if flag_resp==1 %Terminate and record rating on left mouseclick
                    ...
                    rating = round(((Slider_x_pos - (setup.ScrWidth/2-Scale_width/2))/ Scale_width)*Scale_length); %rescaling of scale_width independent of screen resolution [0-200]
                    rating_subm = 1;
                    break
           ...
           if control_joystick == 1
            ...
            elseif control_joystick == 0 && mousebuttons(1) == 1
                ...
                if flag_resp==1 %Terminate and record rating on left mouseclick
                    ...
                    rating = ((Slider_x_pos - (setup.ScrWidth/2-Scale_width/2))/ Scale_width)*Scale_length; %rescaling of scale_width independent of screen resolution [0-100]
                    rating_subm = 1;
                    break
