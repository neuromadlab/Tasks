...
%% --- Start trial--- anticipation
    ...
    trial.question = Question{i_rating}; %sets trial question
...
%Set trial instruction according to TasteTest Phase
%Phase I: anticipation
if strcmp(trial.phase, 'anticipation')
    text.task = 'Beschreiben Sie das Aussehen: \n';
%Phase II: consumption  
elseif strcmp(trial.phase, 'consumption')
    text.task = 'Beschreiben Sie den Geschmack: \n';
else
    text.task = ['Hier liegt ein Fehler (Phase) vor.'...
                'Wenden Sie Sich an die Versuchsleitung'];
end
   
%Setup trial item
if  strcmp(trial.question, 'appetite')
     text.question = 'Wie appetitanregend finden Sie den Snack?';
elseif strcmp(trial.question, 'craving')
   text.question = 'Wie sehr wollen Sie den Snack essen?';
else
   text.question = ['Hier liegt ein Fehler (Frage) vor.'...
                    'Wenden Sie Sich an die Versuchsleitung'];
end
...
% Setup Text positions
    DrawFormattedText(rating_scr, trial.snack, 'center', (Scale_offset+wh/2 - 230), color_scale_anchors,40,[],[],2);
    DrawFormattedText(rating_scr, text.task, 'center', (Scale_offset+wh/2 - 200), color_scale_anchors,40,[],[],2);
