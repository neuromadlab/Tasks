...
if lang_de == 1
    ...
    Question = { 'Wanting', 'Wie sehr wollen Sie den Snack?', 'Wie sehr wollen Sie den Snack?';
        ...
        'Intensität', 'Wie intensiv\n würde der Snack schmecken?', 'Wie intensiv\n schmeckt der Snack?';
        'Süße', 'Wie süß\n würde der Snack schmecken?', 'Wie süß\n schmeckt der Snack?';
        'Säure', 'Wie sauer\n würde der Snack schmecken?', 'Wie sauer\n schmeckt der Snack?';
        'Salzigkeit', 'Wie salzig\n würde der Snack schmecken?', 'Wie salzig\n schmeckt der Snack?';
        'Bitternis', 'Wie bitter\n würde der Snack schmecken?', 'Wie bitter\n schmeckt der Snack?';
        'Umami', 'Wie umami/würzig\n würde der Snack schmecken?', 'Wie umami/würzig\n schmeckt der Snack?'};
    
    mouseclick = 'Weiter mit Mausklick';
    
    single_phase_2 = 'Probieren Sie den Snack.';
    double_phase_2 = 'Probieren Sie den Snack erneut.';
    single_phase_summoning = 'Bitte fahren Sie jetzt mit der Bewertung fort.';
    rating_summoning = 'Bitte geben Sie jetzt Ihre Bewertung ab.';
else
    ...
    if sweet_first
        Snacks = {  'Sweets', 'Gummy Bear', 'sweet', 'a', '[A]';
            ...
            'Raisins', 'Raisins', 'sweet', 'e', '[E]';
            'Nic Nacs', 'Nic Nacs', 'salty', 'f', '[F]'};
    else
        Snacks = { 'Pretzels', 'Pretzels', 'salty', 'a', '[A]';
            'Raisins', 'Raisins', 'sweet', 'b', '[B]';
            'Cracker', 'Cracker', 'salty', 'c', '[C]';
    ...
    single_phase_summoning = 'Please continue now with the rating.';
    rating_summoning = 'Please rate now.';
end
...
for i_phase = 1 : nrP
    ...
    if i_phase == 1
        ...
        if lang_de
            ...
            [pos.x,pos.y,pos.bbox] = DrawFormattedText(w, sprintf(instruct.text, 'the taste'), 'center', pos.y + 50, [0 0 0],60);
        end
        [pos.x,pos.y,pos.bbox] = DrawFormattedText(w, mouseclick, 'center', wh-50, [0 0 0],60);
        Screen('Flip', w, []);
    end
    ...
    for i_snack = 1 : nrS
        ...
        for i = 1 : n
            ...
            if (i_phase == 2 || i_phase == 3) && i == 1
                ...
                tic
                while ~any(buttons) && toc <= max_waiting_snack
                    [x,y,buttons] = GetMouse;
                    until = toc;
                end
                
                if until > max_waiting_snack
                    % summon to go to ratings
                    [pos.x,pos.y,pos.bbox] = DrawFormattedText(w, single_phase_summoning, 'center', 'center', [0 0 0],40);
                    [pos.x,pos.y,pos.bbox] = DrawFormattedText(w, mouseclick, 'center', wh-50, [0 0 0],60);
                    Screen('Flip', w, [])
            ...
            for i_rating = 1 : nrQ
                ...
                elseif i_phase == 2
                    ...
                    output.rating.phase(output_index,1) = i_phase + (i-1);

                % re-code Phase 3 to Phase 4
                else
                    
                    output.rating.phase(output_index,1) = i_phase + i;
                ...
                save(fullfile('Backup', [filename datestr(subj.date,'yymmdd_HHMM') '.mat']),'design','output','subj');
            end
        end
