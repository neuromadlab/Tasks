...
let expName = 'food_choice_v4';  // from the Builder filename that created this script
let expInfo = {
    'participant': 'pilot1',
    'session': '0',
};
...
flowScheduler.add(BerghainRoutineBegin());
flowScheduler.add(BerghainRoutineEachFrame());
flowScheduler.add(BerghainRoutineEnd());
...
async function updateInfo() {
  ...
  psychoJS.experiment.dataFileName = (("." + "/") + `data/${expName}_${studyID}_${expInfo["participant"]}_S${expInfo["session"]}_${expInfo["date"]}`);
...
var textbox_location;
var BerghainClock;
var textbox_Berghain;
...
async function experimentInit() {
  ...
  // prepare some vars
  locationText = "foobar supreme";
  showLocation = false; // flag to control if location shall be viewed
  doShelfExtract = true; // read food_rating from shelf by default
  
  // determine if running locally, online-pilot or online-"running"
  if (window.location.href.includes("localhost")) { // local
      locationText = "LOCAL";
      showLocation = true;
      doShelfExtract = false; // using fix test-set for food_rating
      
  } else if (window.location.href.includes("pilotToken")) { // online-pilot
      locationText = "PILOT";
      showLocation = true;
      doShelfExtract = false; // using fix test-set for food_rating
      
  } else { // online-"running"
      locationText = "Welcome Participant";
      showLocation = false;
  ...
  if(doShelfExtract){
      ...
      food_ratings = [];
      // get id's of all participants from shelf
      existingParticipants = await psychoJS.shelf.getDictionaryFieldNames({
          key: ["food_ratings", "@designer"]});
      console.log(existingParticipants)
          
      // test if there is an entry for the current participant
      hasFoodRating = existingParticipants.includes(expInfo['participant']);
      if(hasFoodRating){
          // if so, get his/her data
  ...
  // fallback food_rating, when we don't read from the shelf
  if(!doShelfExtract){
  ...
  textbox_location = new visual.TextBox({
    win: psychoJS.window,
    name: 'textbox_location',
    text: '',
    ...
    fillColor: undefined, borderColor: 'red',
    languageStyle: 'LTR',
  ...
  // Initialize components for Routine "Berghain"
  BerghainClock = new util.Clock();
  textbox_Berghain = new visual.TextBox({
    win: psychoJS.window,
    name: 'textbox_Berghain',
...
function shelf_extractRoutineBegin(snapshot) {
  return async function () {
    ...
    // in case the text is not shown, the routine doesn't end itself.
    if ( !showLocation ) {
        continueRoutine = false;
    }
    textbox_location.setText(locationText);
    shelf_extractMaxDuration = null
    // keep track of which components have finished
    shelf_extractComponents = [];
    shelf_extractComponents.push(textbox_location);
    
    for (const thisComponent of shelf_extractComponents)
...
function shelf_extractRoutineEachFrame() {
  return async function () {
    ...
    // *textbox_location* updates
    if ((showLocation) && textbox_location.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      textbox_location.tStart = t;  // (not accounting for frame time here)
      textbox_location.frameNStart = frameN;  // exact frame index
      
      textbox_location.setAutoDraw(true);
    }
    
    if (textbox_location.status === PsychoJS.Status.STARTED && t >= (textbox_location.tStart + 3)) {
      textbox_location.setAutoDraw(false);
    }
...
var BerghainMaxDurationReached;
var BerghainMaxDuration;
var BerghainComponents;
function BerghainRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'Berghain' ---
    t = 0;
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    BerghainClock.reset();
    routineTimer.reset();
    BerghainMaxDurationReached = false;
    // update component parameters for each repeat
    // We only show this routine in case we try to get a food_rating from shelf,
    // and there is none.
    continueRoutine = false;
    if ( doShelfExtract && !hasFoodRating ) {
        continueRoutine = true;
    }
    
    psychoJS.experiment.addData('Berghain.started', globalClock.getTime());
    BerghainMaxDuration = null
    // keep track of which components have finished
    BerghainComponents = [];
    BerghainComponents.push(textbox_Berghain);
    
    for (const thisComponent of BerghainComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function BerghainRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'Berghain' ---
    // get current time
    t = BerghainClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    
    // *textbox_Berghain* updates
    if ((doShelfExtract && !hasFoodRating) && textbox_Berghain.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      textbox_Berghain.tStart = t;  // (not accounting for frame time here)
      textbox_Berghain.frameNStart = frameN;  // exact frame index
      
      textbox_Berghain.setAutoDraw(true);
    }
    
    if (textbox_Berghain.status === PsychoJS.Status.STARTED && t >= (textbox_Berghain.tStart + 5)) {
      textbox_Berghain.setAutoDraw(false);
    }
...
function BerghainRoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'Berghain' ---
    for (const thisComponent of BerghainComponents) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    }
    psychoJS.experiment.addData('Berghain.stopped', globalClock.getTime());
    // When there is no food rating on the shelf, the experiment ends
    // with this routine. Goodby;)
    if ( doShelfExtract && !hasFoodRating ) {
        return quitPsychoJS('Thank you for your participation', false);
    }
    
    // the Routine "Berghain" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
...
function filter_imagesRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    // This is the selection algorithm for selecting the 20 images out of 60 for the 
    // choices
    //
    // The range of the liking ratings is assumed to be in range [-100, 100], and 
    // is the relevant variable.
    // Starting with a anchorpoint of 17.82 for the moderate range, choose the 20 
    // images with the lowest distance to it.
    // But no negative values if possible, fill up with values from above when 
    // reaching 0.
    // If there are not enough positive values, fill up with negatives.
    
    
    // Sort each subset of food_ratings alphabetically by the first element, so the 
    // order of ratings is the same for each image.
    ...
    // Sort for liking values, the images with the lowest liking values will be the
    // first in the food_ratings array.
    ...
    // Filter the 20 entries with the closest liking to the moderateAnchor
    const moderateAnchor = 17.82; // anchorpoint for a moderate liking range
    const posDistances = []; // stores food_ratings + distance to anchor for positive likings
    const negDistances = []; // stores food_ratings + distance to anchor for negative likings
    food_ratings.forEach(subarray => { // for each single food rating...
        // get the liking rating
        const likingEntry = subarray.find(entry => Array.isArray(entry) && entry[0] === 'liking');
        const likingValue = likingEntry[1];
        
        // save the food rating together with its distance to the anchorpoint
        const distance= Math.abs(likingValue - moderateAnchor);
        if (likingValue >= 0) {
            posDistances.push({ subarray, distance });
        } else {
            negDistances.push({ subarray, distance });
        }
      });
    posDistances.sort((a, b) => a.distance - b.distance); // sort by distance
    negDistances.sort((a, b) => a.distance - b.distance); // sort by distance
    // add negative likings to end as fallback, if there are less than 20 positives
    const allDistances = posDistances.concat(negDistances);
    // get the 20 food ratings with the smallest distance to the anchor point
    filteredLikings = allDistances.slice(0, 20).map(item => item.subarray);
    ...
    for (const thisComponent of filter_imagesComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function device_selectionRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of device_selectionComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function mobileLandscapeRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of mobileLandscapeComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function questionloopLoopBegin(questionloopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisQuestionloop of questionloop) {
...
function emaType1conditionLoopLoopBegin(emaType1conditionLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType1conditionLoop of emaType1conditionLoop) {
...
function emaType2conditionLoopLoopBegin(emaType2conditionLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType2conditionLoop of emaType2conditionLoop) {
...
function emaType3ConditionLoopLoopBegin(emaType3ConditionLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType3ConditionLoop of emaType3ConditionLoop) {
...
function emaType4conditionLoopLoopBegin(emaType4conditionLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType4conditionLoop of emaType4conditionLoop) {
...
function emaType5ConditionsLoopLoopBegin(emaType5ConditionsLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType5ConditionsLoop of emaType5ConditionsLoop) {
...
function emaType6ConditionLoopLoopBegin(emaType6ConditionLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType6ConditionLoop of emaType6ConditionLoop) {
...
function emaType7ConditionLoopLoopBegin(emaType7ConditionLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType7ConditionLoop of emaType7ConditionLoop) {
...
function emaType8ConditionLoopLoopBegin(emaType8ConditionLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType8ConditionLoop of emaType8ConditionLoop) {
...
function emaType10ConditionLoopLoopBegin(emaType10ConditionLoopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisEmaType10ConditionLoop of emaType10ConditionLoop) {
...
function loop_choiceLoopBegin(loop_choiceLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisLoop_choice of loop_choice) {
...
function emaInstructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaInstructionsComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaControllerRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaControllerComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType1RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType1Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType3RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType3Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType4RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType4Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType5RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType5Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType6RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType6Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType7RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType7Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType8RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType8Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaType10RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaType10Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function emaSkipperRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of emaSkipperComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function choicePreparationsRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of choicePreparationsComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
