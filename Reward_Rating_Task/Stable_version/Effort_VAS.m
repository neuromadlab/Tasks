...
% Define colors
color.white = [255 255 255];%WhiteIndex(setup.screenNum); %with intensity value for white on second screen
...
VAS_time_limit = 1; %set 0 for state ratings, 1 for experiment
wait_rest = 0;
...
if settings.lang_de == 1
    ...
    elseif strcmp(scale_type,'taste')       
        if strcmp(Question{i_rating,1},'Umami')
            scale.anchor_1 = 'am allerstaerksten vorstellbares Umami';
        else
            scale.anchor_1 = ['am allerstaerksten vorstellbare ' lang.question.(language){i_rating,1}];
        end
        scale.anchor_2 = 'sehr stark';
        scale.anchor_3 = 'stark';
        scale.anchor_4 = 'einigermaßen';
        scale.anchor_5 = 'wenig';
        scale.anchor_6 = 'kaum wahrnehmbar';
        scale.anchor_7 = 'keine Empfindung';
    else    
        scale.anchor_1 = ['Ueberhaupt nicht'];
    ...
    elseif strcmp(scale_type,'taste')       
        scale.anchor_1 = ['strongest imaginable ' lang.question.(language){i_rating,1}];
        scale.anchor_2 = 'very strong';
        scale.anchor_3 = 'strong';
        scale.anchor_4 = 'moderate';
        scale.anchor_5 = 'weak';
        scale.anchor_6 = 'barely detectable'; 
        scale.anchor_7 = 'no sensation';
    else    
...
%%Questions   
if  strcmp(trial.question,'wanting')
    if category == 1
        if settings.lang_de == 1
            text_question = ['Wie sehr wollen Sie das in diesem Durchgang Gezeigte essen?'];
        else
            text_question = ['How much do you want to eat the item in this trial?']; %english translation
        end
        
    elseif category == 2
        if settings.lang_de == 1
            text_question = ['Wie sehr wollen Sie das in diesem Durchgang Gezeigte sehen?'];
        else
            text_question = ['How much do you want to view the picture in this trial?']; %english translation
        end
    elseif category == 3
        if settings.lang_de == 1
            text_question = ['Wie sehr wollen Sie das in diesem Durchgang Gespielte hoeren?'];
        else
            text_question = ['How much do you want to listen to the music in this trial?']; %english translation
        end
    end
        
elseif  strcmp(trial.question, 'liking')
    if settings.lang_de == 1
        text_question = ['Bitte bewerten Sie das Praesentierte im Vergleich zu allen bisher in Ihrem Leben erfahrenen Empfindungen.'];
        ...
        text_question = ['Please rate the reward in comparison with all of the sensations you have experienced in your life.']; %english translation
...
elseif strcmp(question_type,'load')
    if settings.lang_de == 1
...
elseif strcmp(question_type,'taste')
...
if strcmp(trial.question, 'liking') % vertical scale
    ...
    DrawFormattedText(rating_scr, anchor.a1, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.000, color.scale_anchors,80);
    %DrawFormattedText(rating_scr, [text_freerating ' sensation'], (setup.ScrWidth/2+20), (Scale_offset - 10) + 20, [250 0 0],80);
    DrawFormattedText(rating_scr, anchor.a2, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.171, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a3, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.278, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a4, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.411, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a5, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.469, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a6, (setup.ScrWidth/2+60), (Scale_offset + 5) + Scale_height * 0.500, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a7, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.530, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a8, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.588, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a9, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.708, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a10, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 0.814, color.scale_anchors,80);
    DrawFormattedText(rating_scr, anchor.a11, (setup.ScrWidth/2+200), (Scale_offset + 5) + Scale_height * 1.000, color.scale_anchors,80);
    DrawFormattedText(rating_scr, text_question, rating_scr_rect_x1, rating_scr_rect_y2 + 320, color_scale_anchors,60,[],[],2);

    % horizontal scale elements
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.000, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.000,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.171, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.171,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.278, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.278,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.411, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.411,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.469, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.469,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +160), Scale_offset + Scale_height * 0.500, (setup.ScrWidth / 2 + 200), Scale_offset + Scale_height * 0.500,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.530, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.530,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.588, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.588,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.708, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.708,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 0.814, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 0.814,3)
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2 +178), Scale_offset + Scale_height * 1.000, (setup.ScrWidth / 2 + 195), Scale_offset + Scale_height * 1.000,3)

    % vertical scale element
    Screen('DrawLine',rating_scr,color.scale_anchors,(setup.ScrWidth / 2+180), Scale_offset, (setup.ScrWidth / 2+180), Scale_offset + Scale_height,3)
    
    Screen('FrameRect', rating_scr, highlighter_colour, rating_scr_rect,3);
    Screen('DrawTexture',rating_scr, texture_i,[], [rating_scr_rect]);
    ...
    %--- Start sound if presenting music stimuli
    if position > 40 && settings.start_phase_I 
        PsychPortAudio('Start', pahandle, 1, 1, 0, [], []); 
    end

    %----Mouse response----

    %Move cursor to anchor_6 (center position of sclae)
    X = round(setup.ScrWidth/2+180);
    Y = round(Scale_offset + Scale_height * 0.50) + jitter_slider(count_jitter_slider); %Fix y coordinate
    count_jitter_slider = count_jitter_slider +1;
    Slider_y_pos = Y;
    ...
    Screen('DrawLine',w,[250 0 0],(setup.ScrWidth / 2 + 170), Slider_y_pos, (setup.ScrWidth / 2 + 190), Slider_y_pos,5)
    ...
    if control_joystick == 1
        ...
        [mouseX, mouseY, mousebuttons] = GetMouse(setup.screenNum); %Find out coordinates of current mouse position
        offset_y = 0;
        proj_y = round(mouseY);
    end
    
    controller_positions = NaN(sampling_time*100,1);
    controller_positions_rating = NaN(sampling_time*100,1);%store controller positions 
    pos_index = 1;
    pos_count = 0;
    while (GetSecs - t_scale_trigger) < max_dur_rating && flag_resp == 0 %mousebuttons(1)~=1 
        ...
        if (GetSecs > nextTime) %Sample every 0.01 seconds
            ...
            if mod(pos_count,10) == 0
                controller_positions(pos_index,1) = Slider_y_pos;
                controller_positions_rating(pos_index,1) = 100-((Slider_y_pos - Scale_offset)/Scale_height)*200;
    ...
    %----- Stop audio if presenting music stimulus -----
    if position > 40 && settings.start_phase_I 
...
elseif strcmp(trial.question,'taste')
    Scale_height = round(setup.ScrHeight * .66);
    %Scale_offset = round(setup.ScrHeight * scale_offset_y);
    Scale_offset = round((setup.ScrHeight - (setup.ScrHeight * .85)) * .75);
    ...
    Screen('TextFont',rating_scr,'Arial');
    DrawFormattedText(rating_scr, scale.anchor_1, (ww/2+20), (Scale_offset + 5) + Scale_height * (scale.y_6+0.02), color_scale_anchors,80);
    %DrawFormattedText(rating_scr, [text_freerating ' sensation'], (ww/2+20), (Scale_offset - 10) + 20, [250 0 0],80);
    DrawFormattedText(rating_scr, scale.anchor_2, (ww/2+20), (Scale_offset + 5) + Scale_height * scale.y_5, color_scale_anchors,80);
    DrawFormattedText(rating_scr, scale.anchor_3, (ww/2+20), (Scale_offset + 5) + Scale_height * scale.y_4, color_scale_anchors,80);
    DrawFormattedText(rating_scr, scale.anchor_4, (ww/2+20), (Scale_offset + 5) + Scale_height * scale.y_3, color_scale_anchors,80);
    DrawFormattedText(rating_scr, scale.anchor_5, (ww/2+20), (Scale_offset + 5) + Scale_height * scale.y_2, color_scale_anchors,80);
    DrawFormattedText(rating_scr, scale.anchor_6, (ww/2+20), (Scale_offset + 5) + Scale_height * scale.y_1, color_scale_anchors,80);
    DrawFormattedText(rating_scr, scale.anchor_7, (ww/2+20), (Scale_offset + 5) + Scale_height * (scale.y_0+0.02), color_scale_anchors,80);


    % horizontal scale elements
    Screen('DrawLine',rating_scr,color_scale_anchors,(ww / 2 - 2), Scale_offset + Scale_height * scale.y_0, (ww / 2 + 15), Scale_offset + Scale_height * scale.y_0,3)
    Screen('DrawLine',rating_scr,color_scale_anchors,(ww / 2 - 2), Scale_offset + Scale_height * scale.y_1, (ww / 2 + 15), Scale_offset + Scale_height * scale.y_1,3)
    Screen('DrawLine',rating_scr,color_scale_anchors,(ww / 2 - 2), Scale_offset + Scale_height * scale.y_2, (ww / 2 + 15), Scale_offset + Scale_height * scale.y_2,3)
    Screen('DrawLine',rating_scr,color_scale_anchors,(ww / 2 - 2), Scale_offset + Scale_height * scale.y_3, (ww / 2 + 15), Scale_offset + Scale_height * scale.y_3,3)
    Screen('DrawLine',rating_scr,color_scale_anchors,(ww / 2 - 2), Scale_offset + Scale_height * scale.y_4, (ww / 2 + 15), Scale_offset + Scale_height * scale.y_4,3)
    Screen('DrawLine',rating_scr,color_scale_anchors,(ww / 2 - 2), Scale_offset + Scale_height * scale.y_5, (ww / 2 + 15), Scale_offset + Scale_height * scale.y_5,3)
    Screen('DrawLine',rating_scr,color_scale_anchors,(ww / 2 - 2), Scale_offset + Scale_height * scale.y_6, (ww / 2 + 15), Scale_offset + Scale_height * scale.y_6,3)
    ...
    %Move cursor to anchor_6 (center position of scale)
...
elseif strcmp(trial.question, 'willingnesspay') % horizontal scale with image for willingness to pay
    ...
    DrawFormattedText(rating_scr, text_question, 'center', (Scale_offset+setup.ScrHeight/2 - desired_img_height*5), color.scale_anchors,80,[],[],2);
...
%     Screen('DrawTexture',rating_scr,texture_i,[],[(setup.ScrWidth/2-Scale_width/3) (Scale_offset+setup.ScrHeight/2-500) (setup.ScrWidth/2+Scale_width/3) (Scale_offset+setup.ScrHeight/2-50)]);
    
    %Screen('DrawTexture',rating_scr,texture_i,[], [(setup.ScrWidth/2-Scale_width/(desired_img_width/desired_img_height)) (Scale_offset+setup.ScrHeight/2-desired_img_width*2.5) (setup.ScrWidth/2+Scale_width/(desired_img_width/desired_img_height)) (Scale_offset+setup.ScrHeight/2-desired_img_width*0.25)]);
    Screen('DrawTexture',rating_scr,texture_i,[], [(setup.ScrWidth/2-desired_img_width*2) (Scale_offset+setup.ScrHeight/2-desired_img_height*5+50) (setup.ScrWidth/2+desired_img_width*2) (Scale_offset+setup.ScrHeight/2-desired_img_height+50)]);
 ...
 %--- Start display for trial---
    ...
    X = round(setup.ScrWidth/2) + jitter_slider(count_jitter_slider);
    count_jitter_slider = count_jitter_slider +1;
    ...
    while (GetSecs - t_scale_trigger) < max_dur_rating && flag_resp == 0 %mousebuttons(1)~=1 
        ...
        if (GetSecs > nextTime) %Sample every 0.01 seconds
           if control_joystick == 1
            ...
            if control_joystick == 1 && Joystick.Button(1) == 1
                flag_resp = 1;
                t_button = GetSecs;
                Joystick.Button(1) = 0;
            elseif control_joystick == 0 && mousebuttons(1) == 1
                flag_resp = 1;
                t_button = GetSecs;
                mousebuttons(1) = 0;
            end
    ...
    %Scale_offset = round((wh - (wh * .95)) * .75);
    %DrawFormattedText(rating_scr, text_question, 'center', (Scale_offset+setup.ScrHeight/2 - 100), color.scale_anchors,70,[],[],2);
    DrawFormattedText(rating_scr, text_question, 'center', (Scale_offset+setup.ScrHeight/2 - 100), color.scale_anchors,80,[],[],2);
    ...
    % Screen('DrawTexture',rating_scr, texture_i,[], [rating_scr_rect]);
    % Screen('DrawTexture',rating_scr, texture_i,[], [(rating_scr_rect_x2 - desired_img_width) rating_scr_rect_y1 rating_scr_rect_x2 rating_scr_rect_y2]);
    Screen('DrawTexture',rating_scr,texture_i,[], [(setup.ScrWidth/2-desired_img_width/1.5) (Scale_offset+setup.ScrHeight/2-desired_img_height*2.5) (setup.ScrWidth/2+desired_img_width/1.5) (Scale_offset+setup.ScrHeight/2 - 200)]);
...
%     scale_joy_y = setup.ScrHeight*0.7/JoystickSpecification.Max;
    ...
    controller_positions = NaN(sampling_time*100,1); %store controller positions of first 2 seconds
    pos_index = 1;
    pos_count = 0;
    while (GetSecs - t_scale_trigger) < max_dur_rating && flag_resp == 0 %mousebuttons(1)~=1 
