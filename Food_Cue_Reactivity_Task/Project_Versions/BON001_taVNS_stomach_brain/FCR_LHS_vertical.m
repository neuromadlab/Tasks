...
%
%Update May, 2024, Jana Lieberz:
%updated mouse output (reaction times, submission status)
%========================================================
...
if settings.lang_de == 1
    ...
    anchor_11 = ['am allerstaerksten zuwidere Empfindung, die vorstellbar ist'];
    instruction = ['Bitte bewerten Sie die Belohnung \nim Vergleich zu allen bisher in \nIhrem Leben erfahrenen Empfindungen.'];
else

    instruction = ['Please rate the reward in comparison with all of the sensations you have experienced in your life.']; %english translation
    ...
    anchor_6 = ['neutral'];
    anchor_7 = ['slightly dislike' ];
    anchor_8 = ['moderately dislike' ];
    anchor_9 = ['dislike very much' ];
    anchor_10 = ['extremely dislike' ];
...
DrawFormattedText(rating_scr, anchor_7, (ww/2+20), (Scale_offset - 10) + Scale_height * 0.530 + imsc, color_scale_anchors,80);
DrawFormattedText(rating_scr, anchor_8, (ww/2+20), (Scale_offset - 10) + Scale_height * 0.588 + imsc, color_scale_anchors,80);
DrawFormattedText(rating_scr, anchor_9, (ww/2+20), (Scale_offset - 10) + Scale_height * 0.708 + imsc, color_scale_anchors,80);
DrawFormattedText(rating_scr, anchor_10, (ww/2+20), (Scale_offset - 10) + Scale_height * 0.814 + imsc, color_scale_anchors,80);
DrawFormattedText(rating_scr, anchor_11, (ww/2+20), (Scale_offset - 10) + Scale_height * 1.000 + imsc, color_scale_anchors,80);
    ...
    while (GetSecs - starttime) < timing.max_dur_rating && flag_resp == 0 %mousebuttons(1)~=1
        ...
        if (GetSecs > nextTime) %Sample every 0.01 seconds
            ...
            if mousebuttons(1)==1 || mousebuttons(3) == 1 %Terminate and record rating on left mouseclick
                ...
                output.data(i_trial,value_column) = 100-((Slider_y_pos - Scale_offset)/Scale_height)*200; %rescaling of scale_height independent of screen resolution [-100 to +100]
