...
study.pilot = 0; %set 1 to create randomization for pilot

% load Food and Non-Food Tables, with indices of stimuli to be used  
load('X:\Tasks\FCR\Project_Versions\BON001\Documentation\BON001_FCR_Food_Stimuli.mat');
load('X:\Tasks\FCR\Project_Versions\BON001\Documentation\BON001_FCR_NonFood_Stimuli.mat');
...
for img = 1:length(stimuli_all)
    if startsWith(string(stimuli_all(img)),'99')
...
for n = 1:study.participants
    ...
    for item = 1:size(design.rand.order_mat,1)
        if ~isempty(find(FCR_Food_Stimuli.ImageNr == long_images_rand(item), 1))
            FoodCategory_string = FCR_Food_Stimuli.sweet(find(FCR_Food_Stimuli.ImageNr == long_images_rand(item)));
            if FoodCategory_string == 1
                FoodSweet(item,1) = 1;
            elseif FoodCategory_string == 0
                FoodSweet(item,1) = 0;
            end
            
            Calorie_nr = FCR_Food_Stimuli.kcal(find(FCR_Food_Stimuli.ImageNr == long_images_rand(item)));
            kcal(item,1) = Calorie_nr;
            fat(item,1) = FCR_Food_Stimuli.fat(find(FCR_Food_Stimuli.ImageNr == long_images_rand(item)));
            carbs(item,1) = FCR_Food_Stimuli.carbs(find(FCR_Food_Stimuli.ImageNr == long_images_rand(item)));
 
        else % Non-Food Item, hence, no classification
            FoodSweet(item,1) = NaN;
            kcal(item,1) = NaN;
            carbs(item,1) = NaN;
            fat(item,1) = NaN;
        end
    ...
    design.rand.order_mat(:,4) = FoodSweet;
    design.rand.order_mat(:,5) = kcal;
    design.rand.order_mat(:,6) = fat;
    design.rand.order_mat(:,7) = carbs;
    
    design.rand.order_mat_labels = ["row_index","repetition", "img_index", "sweet", "kcal","fat","carbs"]';
