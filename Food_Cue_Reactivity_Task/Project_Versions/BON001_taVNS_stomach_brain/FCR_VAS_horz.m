...
%--- Start trial---
    ...
    if  strcmp(trial.question,'wanted')
        if settings.lang_de == 1
            ...
            text_question = 'How much do you want to receive the reward in this trial?';
        end
    else
    ...
    DrawFormattedText(rating_scr, [anchor_1 ], (Scr_Width/2-Scale_width/2 - 65), (Scale_offset+wh/2 + 70), color_scale_anchors,80);
    %DrawFormattedText(rating_scr, [anchor_4], 'center', (Scale_offset+wh/2 + 30), [205 201 201],80);
    DrawFormattedText(rating_scr, [anchor_7 ], (Scr_Width/2+Scale_width/2 - 75), (Scale_offset+wh/2 + 70), color_scale_anchors,80);
    ...
    while (GetSecs - starttime) < timing.max_dur_rating && flag_resp == 0 %mousebuttons(1)~=1 
        ...
        if (GetSecs > nextTime) %Sample every 0.01 seconds
            ...
            if mousebuttons(1)==1 || mousebuttons(3) == 1 %Terminate and record rating on left mouseclick
                ...
                output.data(i_trial,value_column) = ((Slider_x_pos - (Scr_Width/2-Scale_width/2))/ Scale_width)*100; %rescaling of scale_width independent of screen resolution [0-100]
            ...
            elseif (mouseX ~= Slider_x_pos) %Update screen if participant has scrolled left or right
                Screen('CopyWindow',rating_scr,w);
                Slider_x_pos = (mouseX);
                ...
                output.data(i_trial,value_column)= ((Slider_x_pos - (Scr_Width/2-Scale_width/2))/ Scale_width)*100;
...
elseif input_type == 1
   ...
   if flag_resp == 0
       output.data(i_trial,value_column) = ((Slider_x_pos - (Scr_Width/2-Scale_width/2))/ Scale_width)*100; %changed
       output.data(i_trial,submission_column)  = 0; % no submission 
       output.data(i_trial,RT_column) = NaN; % no RT 
   end 
   WaitSecs(timing.feedback_delay); %Show screen for 1.5s post-mouseclick
