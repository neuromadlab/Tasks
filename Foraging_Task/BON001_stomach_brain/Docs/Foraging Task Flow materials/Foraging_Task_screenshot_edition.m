function Foraging_Task_screenshot_edition
try
study   = 'BON001';
task    = 'Foraging';
version = '0.2.8';
...
doJoystick  = 1;    % 1 for yes we use a joystick, 0 else
doGFD       = 0;    % 1 for yes we use a GFD, 0 else
doMRITrigger = 0;   % 1 for yes we wait for MRI trigger
...
if isTrain
    ...
    if output.doLongTraining
        path = [ptb.supplPath 'ConditionFiles\cond_longTraining'];
    else
        path = [ptb.supplPath 'ConditionFiles\cond_shortTraining'];
    end
    ...
    path = [ptb.supplPath 'ConditionFiles\cond_'...
        subj.ids '_' num2str(subj.sess) '.mat'];
end
...
%     load(['..' filesep '..' filesep 'EAT' filesep 'data' filesep ...
%         'TrainEAT_' subj.study '_'  subj.subjectID '_S' ...
%         subj.sessionID '_R1.mat'], 'input_device') % old path
    load(['C:\Users\mrt\Desktop\BON001\NIMG\EAT\data\' ...
        'TrainEAT_BON003_900001_S1_R1.mat'], 'input_device') % test path
%     load(['..' filesep '..' filesep 'Effort' filesep 'data' filesep ...
%         'TrainEAT_' subj.study '_'  subj.subjectID '_S' ...
%         subj.sessionID '_R1.mat'], 'input_device') % path BON001 laptop
%     load(['C:\Users\mrt\Desktop\BON001\NIMG\EAT\data\TrainEAT_' ...
%         subj.study '_'  num2str(subj.id) '_S' num2str(subj.sess) ...
...
for i = 1:last     % show the selected instructions
    ...
    DrawFormattedText(ptb.w, text, 'center','center', txtCol, 40, ...
        [],[], 1.3);
...
persistent lastSamples; % to memory the last samples
...
% Second, draw a random sample
pop = shuffle(population, 2);
sample = pop{1};

% we don't want so much coincidence in training, max. 2 repetitions
if output.doTraining
    if strcmp(sample, lastSamples{1}) && strcmp(sample, lastSamples{2})
        % if the current sample would be the third repetition, then...
        sample = pop{2}; % ... choose the other one
    end

    % update sample-memory
    lastSamples{1} = lastSamples{2};
    lastSamples{2} = sample;
end
...
if output.doTraining
    subId = [output.task '_' output.studyID '_' subj.ids '_training'];
else
...
% pressed. If so, it closes the PTB screen and saves data. Else, it behaves
...
lt.instr3.de = ['Wenn Sie diese Aliens fangen, erhalten Sie unterschiedliche Mengen an Treibstoff. Jedoch braucht das Fangen auch unterschiedlich viel Zeit.' ...
...
lt.instr3.eng = ['If you catch these aliens, they will provide you with different amounts of fuel. But they also take different amounts of time to catch.' ...
...
lt.instr7.de = ['2) Halten Sie' device 'links bzw. rechts gedrückt und das Alien wird größer.\nWenn Sie das Alien erfolgreich gefangen haben, wird es aufhören größer zu werden und Sie können loslassen.' ...
    ' Danach sehen Sie, wie viel Treibstoff das Alien Ihnen zur Verfügung gestellt hat. Beachten Sie auch, dass die Aliens schnell erscheinen - entscheiden Sie schnell!'];
lt.instr7.eng = ['2) Keep the' input 'left or right pressed and the alien will get bigger.\n\nWhen you have successfully captured the alien, the alien will stop getting bigger and you can release.' ...
    ' Afterwards you will see how much fuel the alien has provided you with. Also note that aliens appear quickly - you need to be fast to make a choice!'];
...
lt.instr9.de = ['OK. Versuchen wir es mit einem Training, um Sie mit der Aufgabe vertraut zu machen.' ...
    ' Der Treibstoff, den Sie bei diesem Training erhalten, wird nicht auf Ihre Bonuszahlung angerechnet - dies ist nur zur Übung.'];
...
lt.instrS1.de = 'In der nun folgenden Übung haben Sie das Kommando über ein Raumschiff und müssen durch das Sammeln von Aliens Treibstoff beschaffen.';
lt.instrS2.de = ['Wenn Sie das richtige Spiel beginnen, gibt es 4 verschiedene Arten von Aliens, zur Übung aber nur 2. Bei einigen dieser Aliens dauert es länger sie zu fangen als andere' ...
...
lt.instrS5.de = ['Anstatt die Tasten auf der Tastatur zu benutzen, haben Sie jetzt zwei Griffe. Das Prinzip der Aufgabe bleibt das gleiche.' ...
    ' Der einzige Unterschied ist, dass Sie zum Einfangen und Ausweichen der Aliens den jeweiligen Griff drücken müssen.' ...
    ' Denken Sie daran: wenn das Alien nicht mehr grösser wird, können Sie loslassen.'];
lt.instrS6.de = ['Das bedeutet, dass Sie den rechten Griff drücken können, wenn Sie ein Alien auf der rechten Seite fangen wollen und den linken drücken, wenn Sie ein Alien auf der linken einfangen wollen.' ...
    ' Beim Ausweichen eines Alien benutzen Sie den entgegengesetzten Griff. Sie müssen einen gewissen Druck ausüben, um die Aliens zu fangen.'];
...
lt.instrS2.eng = ['When you start the game for real there will be 4 different types of aliens, but only 2 during this practice. Some of these types will take longer to capture than others.' ...
...
lt.instrS5.eng = ['Instead of using the keys on the keyboard, you now have two handles. The principle of the task stays the same.' ...
    ' The only difference is that you have to press the respective handle for capturing and avoiding the aliens. Remember: when the alien stops to grow, you can release the handle.'];
lt.instrS6.eng = ['This means you can press the right handle when you want to capture an alien on the right side and press the left side when capturing an alien on the left side.' ...
    ' When avoiding an alien you use the opposite handle. You have to exert a certain amount of pressure to catch the aliens.'];
...
lt.train2_1.de = ['Manche Arten von Aliens sind besser als andere. Im letzten Training, das Sie gemacht haben, haben Sie vielleicht bemerkt, dass es zwei Aliens gab.' ...
    ' Eins von ihnen war schnell zu fangen und gab Ihnen eine Menge Treibstoff. Das andere Alien brauchte lange, um gefangen zu werden, und lieferte nicht viel Treibstoff.'];
lt.train2_1.eng = ['Some types of aliens are better than others. In the training you did, you may have noticed that there were two aliens.' ...
    ' One of these was quick to capture and gave you a lot of fuel. The other alien took a long time to capture and did not provide you with much fuel.'];
...
lt.train3_1.de = ['Am Ende der Aufgabe wird die Menge an Treibstoff, die Sie gesammelt haben, in eine Bonuszahlung umgewandelt.' ...
    ' Je mehr Treibstoff Sie in Ihrer Zeit sammeln, desto höher wird Ihre Bonuszahlung sein! 1 Liter steht für viel Geld.' ...
...
lt.train3_1.eng = ['At the end of the task, the amount of fuel you collected will be converted into a bonus payment.' ...
    ' The more fuel you get during your time, the higher your bonus payment will be! 1 liter stands for much Money.' ...
