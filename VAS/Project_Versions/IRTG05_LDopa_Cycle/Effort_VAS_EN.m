...
if  strcmp(trial.type,'Influenca_1')
    text_question = ['How satisfied are you with yourself right now?'];
    scale.anchor_1 = ['Not at all'];
    scale.anchor_7 = ['Very'];
    
    
elseif  strcmp(trial.type,'Influenca_2')
    text_question = ['How are you feeling psychologically at the moment?'];
    scale.anchor_1 = ['Very bad'];
    scale.anchor_7 = ['Very good'];
    
elseif  strcmp(trial.type,'Influenca_3')
    text_question = ['How is your physical condition at the moment?'];
    scale.anchor_1 = ['Very bad'];
    scale.anchor_7 = ['Very good'];
    
elseif strcmp(trial.type,'State_1')
    text_question = ['How ' trial.question  ' are you at the moment?'];
    scale.anchor_1 = ['Not at all'];
    scale.anchor_7 = ['Very much'];
...
DrawFormattedText(rating_scr, text_question, 'center', (Scale_offset+setup.ScrHeight/2 - 150), color.scale_anchors,40,[],[],2);
   ...
   SetMouse(X,Y)
    ...
    %[Joystick.X, Joystick.Y, Joystick.Z, Joystick.Button] = WinJoystickMex(JoystickSpecification.Handle);
    %offset_x = X - round(mouseX * scale_joy_x);
    %proj_x = round(mouseX * scale_joy_x) + offset_x;
