...
% Project Version for IRTG05 (Zsofia Karlocai,adapted from Corinna Schulz, Dec 2023:
...
do_gamepad = 0; %0 for touchscreen on tablet
...
subj.sessionID = input('Session ID [C1,C2, S1, S2]: ','s');
subj.tID = input('Runs [1 2]: ','s');
subj.lang = input('Language [1 for German, 0 for English]: ','s');
...
if strcmp(subj.lang,'1')
    text_Cont = ['Weiter mit Mausklick.'];
elseif strcmp(subj.lang,'0')
...
elseif strcmp(subj.lang,'0')
    text = ['Welcome. \n\n Below you will answer a few questions about how you feel currently.'];
end
...
%Instruction text II                                               
if strcmp(subj.lang,'1')
    text = ['Um Ihre Antworten einzugeben koennen Sie einen Regler ueber eine Skala verschieben. Bewegen Sie den Regler mit dem Mousepad (oder Touchscreen) und bestaetigen Sie Ihre Eingabe mit einem Mouse-Click (Touchscreen: Doppelklick).'];
elseif strcmp(subj.lang,'0')
    text = ['To answer the questions, you will need to move a slider over a scale. Move the slider with the mouse pad (or touch screen) and confirm your entry with a mouse click (touch screen: double click).'];
end
Screen('TextSize',w,32);
Screen('TextFont',w,'Arial');
...
state_questions = {  'hungry', 'hungrig', 'State_1';
                     'thirsty', 'durstig', 'State_1';    
                     'tired', 'muede', 'State_1';
                     'full', 'satt', 'State_1';
                     'masculine', 'maskulin', 'State';
                     'feminine', 'feminin', 'State';
                     'selfsatisfaction', 'selbstzufrieden', 'Influenca_1';
                     'physical condition' , 'koerperliches Befinden', 'Influenca_2';
                     'mental condition' , 'psychisches Befinden', 'Influenca_3';
                     %'awake', 'wach', 'State';
                     'active', 'aktiv', 'State';
                     'distressed', 'bedrueckt', 'State';
                     'interested', 'interessiert', 'State_1';
                     'excited', 'freudig erregt', 'State_1';
                     'upset', 'veraergert', 'State_1';
                     'strong', 'stark', 'State';
                     'guilty', 'schuldig', 'State';
                     'scared', 'veraengstigt', 'State_1';
                     'hostile', 'feindselig', 'State';
                     'inspired', 'angeregt', 'State';
                     'proud', 'stolz', 'State';
                     'irritable', 'reizbar', 'State_1';
                     'enthusiastic', 'begeistert', 'State_1';
                     'ashamed', 'beschaemt', 'State';
                     'alert', 'hellwach', 'State_1';
                     'nervous', 'nervoes', 'State_1';
                     'determined', 'entschlossen', 'State_1';
                     'attentive', 'aufmerksam', 'State_1';
                     'jittery', 'unruhig', 'State';
                     'afraid', 'aengstlich', 'State_1'};
...
for i_state = 1:length(state_questions) 
       ...
       if strcmp(subj.lang,'1') % German
        trial.question = state_questions{i_state,2};% Question Item
        trial.type = state_questions{i_state,3};% State or Caloric Load
       elseif strcmp(subj.lang,'0') % English
        trial.question = state_questions{i_state,1};
        trial.type = state_questions{i_state,3};
       end
    
%     if  strcmp(trial.type,'Caloric_wanting') || strcmp(trial.type,'Caloric_liking')
%         Caloric_Load
%     else 
if strcmp(subj.lang,'1') % German, call code with German questions
      Effort_VAS
elseif strcmp(subj.lang,'0') % English, call code with English questions
    Effort_VAS_EN
end
%     end 
    ...
    filename = ['VASstate_', subj.study, '_', subj.subjectID, '_', subj.sessionID, '_R', subj.tID];
    output.filename = fullfile(pwd, filesep, 'Data', filesep, filename);
    
    save([output.filename '.mat'], 'output', 'subj', 'state_questions', 'lang')
...
save(fullfile('Data', [filename '.mat']), 'output', 'subj', 'state_questions', 'lang');
...
elseif strcmp(subj.lang,'0')
    text = ['We are now done with the questions. Please refer to the experimenter.'];
end
Screen('TextSize',w,32);
Screen('TextFont',w,'Arial');
[pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, text, 'center', (setup.ScrHeight/5), color.black, 50, [], [], 1.2);
