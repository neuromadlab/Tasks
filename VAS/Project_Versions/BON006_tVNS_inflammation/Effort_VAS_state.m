...
subj.sessionID = input('Session [1, 2, 3 or 4]: ','s');
...
while correct_sess ~= 1
    ...
    elseif str2double(subj.sessionID) == 3
        correct_sess = input(['The specified session is ' subj.sessionID ', indicating the third session. Is this correct (1) or not (0)?: ']);
    elseif str2double(subj.sessionID) == 4
        correct_sess = input(['The specified session is ' subj.sessionID ', indicating the third MRI session, fourth session in total. Is this correct (1) or not (0)?: ']);
    elseif ~ismember(str2double(subj.sessionID), 1:3)
        fprintf('\nThe specified session is not a valid session for BON006! ');
    end

    if correct_sess ~= 1
        subj.sessionID = input('Please enter the correct session ID [1, 2, 3 or 4]: ','s');
    end
end
...
while correct_run ~= 1
    ...
    elseif str2double(subj.runID) == 2
        correct_run = input(['The specified run is ' subj.runID ', indicating the second run. Is this correct (1) or not (0)?: ']);
    ...
    elseif ~ismember(str2double(subj.runID), 1:2)
        fprintf('\nThe specified run is not a valid run for BON006. ');
    end
