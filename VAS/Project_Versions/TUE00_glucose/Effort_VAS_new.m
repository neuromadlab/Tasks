...
if settings.lang_de == 1
    ...
    elseif strcmp(scale_type,'fcqtr_1')
        scale.anchor_1 = ['absolutely not'];
        scale.anchor_7 = ['extremely good'];

    elseif strcmp(scale_type,'fcqtr_2')
        scale.anchor_1 = ['never'];
        scale.anchor_7 = ['always'];

    elseif strcmp(scale_type,'load_shake')||strcmp(trial.type,'load_muesli')
        ...
        while (GetSecs - startTime) < max_dur_rating && flag_resp == 0 %mousebuttons(1)~=1
            ...
            if (GetSecs > nextTime) %Sample every 0.01 seconds
                ...
                if mousebuttons(1)==1 || mousebuttons(3)==1 %Terminate and record rating on left mouseclick
                    rating =  100-((Slider_y_pos - Scale_offset)/Scale_height)*200; %rescaling of scale_width independent of screen resolution [0-100]
                    rating_subm = 1;
