...
subj.id = str2double(subj.subjectID); %converts Session ID to integer
...
%    [pos.text.x,pos.text.y,pos.text.bbox] = DrawFormattedText(w, text_Cont, 'center', (setup.ScrHeight/5*4.7), color.black, 50, [], [], 1.2);
    ...
    if any(mouse.buttons)
       output.rating(i_level, 1) = subj.id; 
       output.rating(i_level, 2) = subj.sess; 
       output.rating(i_level, 3) = subj.t; 
       output.rating(i_level,4) = GetSecs; %Start time of rating
        Effort_VAS
            
        output.rating(i_level,5) = rating; %rating value
        output.rating(i_level,6) = i_level; %rating label code (index of state_questions cell array)
        output.rating(i_level,7) = rating_subm;  % answer submitted by pressing A
        output.rating(i_level,8) = t_rating_ref; %Time of rating submission
        
        output.ratingLabels = {'ID','Session','Timepoint','Time_StartRating','Rating','Question_Index','Submitted','RT'};
       ...
       % filename = ['VASstim_', subj.studyID, '_', subj.subjectID, '_S', subj.sessionID];
       % output.filename = fullfile(pwd, filesep, 'data', filesep, filename);

        output.filename = sprintf('%s\\data\\VASpain_%s_%s_S%s_temp', pwd, subj.studyID, subj.subjectID, subj.sessionID);
...
%output.filename = sprintf('VASstim_%s_%s_%s_%s', subj.studyID, subj.subjectID, subj.sessionID);

output.filename = ['VASpain_', subj.studyID, '_', subj.subjectID, '_S', subj.sessionID];
%output.filename = fullfile(pwd, filesep, 'Data', filesep, filename);
