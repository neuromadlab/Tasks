...
% Project Version for TUE012 (Kristin Kaduk, April 2024:
...
%% FOllowing chunk has been added in Nov. 2022
path_Checkup = strrep(cd, '\', '\\');
reply = input(['\n ', path_Checkup,  '\n Is the name (e.g. TUE012) of the study included in the path displayed here in the CommandWindow?  ',  ' Answer: y/n ']  ,'s');
if strcmp(reply, "y")|| strcmp(reply, "Y")
    disp('Please continue.')
elseif strcmp(reply, "n")|| strcmp(reply, "N")
    error('Please change the folder to  Desktop/TUE012 and open the script Effort_VAS_state.m  for the TUE012 study')
else
    error('There was no identifiable input to the question. Try again by runnning the script again. ')
end
...
subj.tID = input('timepoint [1 2 3 4]: ','s');
...
state_questions = {  'hungry', 'hungrig', 'State';
                     ...
                     'tired', 'müde', 'State';
                     'full', 'satt', 'State';
                     'stressed', 'gestresst', 'State'; 
                     'fatigue', 'erschöpft', 'State'; 
                     'down', 'niedergeschlagen', 'State';
                     'motivated', 'motiviert', 'State';
                     'tense', 'angespannt', 'State';
                     'challenged', 'überfordert', 'State';
...
if ~isnan(subj.caloricLoad)   
    state_questions = [ {'drink_wanting','drink_wanting','Caloric_wanting'};
                        {'drink_liking','drink_liking','Caloric_liking'};
                        state_questions];
end
...
%save(output.filename, 'output', 'subj', 'state_questions');
