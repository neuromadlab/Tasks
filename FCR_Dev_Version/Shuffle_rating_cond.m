...
if selection_exists == 0
    ...
    % Manually select  which photos to use in current run
...
%% Create order scripts
    ...
    for ind = 1:n_subj
        ...
        % randomization for 1 rep
        % prepare empty matrix for random wanting/liking
        full_flip_coin = [];

        for ind_flip = 1:(n_reps/2) %1 run ocntains 2 reps
            % create vector of 1 and 0 with total length of n_trilas (60)
            flip_coin = [zeros((n_trials/2),1); ones((n_trials/2),1)];
            % randomize order of 1 and 0
            flip_coin = Shuffle(flip_coin);
            % add second colum mirroring values (=2nd rep)
            run_flip_coin = [flip_coin (1 - 1 .* flip_coin)];
            % add vectors to full matrix of boolean (results in 60x4)
        ...
        for ind_flip = 1:(n_reps/2)
            % shuffle image vector -> get image name and index as output
            [rand_image_ind, order] = Shuffle(image_index); % 2 outputs
            % shuffle image vector again fo second run
            order_r2 = Shuffle(order);
            % add shuffled image vector (file ID) to 60x4 matrix, reps in colums
            rand_image_mat(:,ind_flip*2-1) = image_index(order);
            rand_image_mat(:,ind_flip*2) = image_index(order_r2);
            % add shuffled image vector (file name) to 60x4 cell
            rand_image_file(:,ind_flip*2-1) = image_files(order);
            rand_image_file(:,ind_flip*2) = image_files(order_r2);
            
            % BIS HIER W?RE ES FALSCH, ABER NACHVOLLZIEHBAR:
            % 60x4 MATRIX MIT RANDOMISIERTEN WANTING/LIKING RATINGS ('full_flip_coin')
            % 60x4 MATRIX MIT RANDOMISIERTEN BILDER INDICES ('rand_img_mat)
            % => KLAR, DASS DANN MANCHE BILDER NICHT GLEICH VIELE LIKING
            % RATINGS BEKOMMEN; WENN DIE UNABH?NGIG VONEINANDER RANDOMISIERT SIND...
            
            %DESHALB: WAS GENAU PASSIERT HIER???
            % (Ich glaube das war auch die Stelle wo Chloe ziemlich viel
            % rumgebastelt hat, ich hab es nicht ausreichend nachvollziehen k?nnen)
            
            % get wanting/liking randomization for following rep
        ...
        ((((i_rep-1)*n_trials)+1):(i_rep*n_trials),5) = i_rep;
            for i_img = 1:n_trials
              ...
              case 'salt_lc'
                 img_spec = [1 0 0]; % food / not sweet / not hcal
              case 'sweet_hc'
 ...
 design.rand.order_mat_labels = [1:9;...
        ...
        if Shuffle_Cond_Version == 1
                save(['Order_TUE002/FCRcond_mat_TUE002_', subj_ind, '.mat'],'design')
        else %Shuffle_Cond_Version == 2 
                save(['Order_TUE002_NF/FCRcond_mat_TUE002_', subj_ind, '.mat'],'design')
        end
