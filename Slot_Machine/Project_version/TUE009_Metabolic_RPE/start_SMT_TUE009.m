%%===================PTB collection===================
%Machine Slot Task - Sara Parnell, Sophie Müller
...
subj.studyID='TUE009';
if settings.debug == 1
    subj.subjectID='900010';
    subj.sessionID='1';
    subj.runID='1';
    subj.lang_de = 1;
    i_rep = input('Would you like to start the task in the second half (only block 3&4)? [0 = no, 1 = yes] ','s');
else
...
if str2num(i_rep) == 1
    ...
    %try to to find the last backup file 
       
    files = dir(['backup\SM_TUE009_',subj.subjectID,'_S',subj.sessionID,'_R',subj.runID,'_*.mat']);
    load([files(end).folder,filesep,files(end).name])
    first_block = 3;
...
if str2num(subj.runID) == 1 && str2num(i_rep) == 0
    %Main instructions
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');
