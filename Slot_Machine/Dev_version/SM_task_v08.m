...
%%
settings.debug = 1; %debug == 0 when in scanner (waits for triggers)

%get input from the MATLAB console
subj.version='TUE009_v1.0';
subj.studyID='TUE009';
if settings.debug == 1
    ...
    subj.sessionID=input('Session ID [5/6]: ','s');
    subj.runID=input('Run ID [1/2]: ', 's');
    subj.lang_de = input('German (1) or English (0): ');
    subj.sessionID = num2str(str2double(subj.sessionID) - 4);
end
...
%set_up trials and blocks: TUE009 4 blocks with 48 trials each
if settings.debug == 1    
    setup.nblocks = 1;
    setup.ntrials = 4;
...
if SMsettings.settings.do_fmri == 1
    % load jitters
   %load('jitters.mat');
   option_duration = 2.35 + jitter_isi(:,1:setup.ntrials); % jitter 2.5+(1.25 max 6) for after choice the logged answer is shown until the spin vid (-0.15 for loading)
   spin_duration = 1.0; % + jitter_isi(:,1:setup.ntrials); % jitter 1.25+(0.5 max 4)
   iti_duration = 1.15 + jitter_iti(:,1:setup.ntrials); % jitter 1+(3 max 12) (+0.15 to compensate for loading missing isi)
   ...
   %code for generating jitters
    ...
    Beh_timings.interval = 5;
    Beh_timings.iti = 1;
end


% random gaussian walk
% rgw_path = 'Conditions\TUE009_rgw_001.mat';
% setup.rgw = load(rgw_path);

%output.rgw_prob = setup.rgw.allprob;
run_nr = (((str2double(subj.sessionID)-1)*2) + str2double(subj.runID));

output.rgw_prob = setup.rgw.allprob(run_nr,:);

%setup.prob = prob;
%genereate rewards for each trial
if strcmp(subj.studyID,'TUE009')
    %     load('rewardTUE009.mat')
else
...
if setup.ntrials == 48 && setup.nblocks ==4
...
if SMsettings.settings.lang_de == 1 && SMsettings.settings.do_fmri
    ...
    setup.instr_decision_txt = 'Nachdem sie den Spielautomaten ausgewaehlt haben, erscheint um den entsprechenden Automaten ein gr??nes Rechteck.\n Sie koennen Ihre Wahl nun nicht mehr aendern.\n\nDruecken Sie einen beliebigen Knopf, um fortzufahren.';
    setup.instr_fast = 'Bitte antworten Sie zuegig, ohne zu lange nachzudenken.\nSie haben etwa 2 Sekunden Zeit zum Antworten.';
...
DrawFormattedText(w, setup.intro_txt, 'center', 'center', color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);
Screen('Flip',w);
WaitSecs(1);
if SMsettings.settings.do_fmri
    while true
...
if str2num(subj.runID) == 1
    %Main instructions
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');
    DrawFormattedText(w, setup.long_instr_txt, 'center', 'center', color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical,1.5);
    Screen('Flip',w);
    WaitSecs(1);
    if SMsettings.settings.do_fmri
        while true
    ...
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');
    DrawFormattedText(w, setup.instr_mach_txt, 'center', setup.yCen*0.2, color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);
    ...
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');
    DrawFormattedText(w, setup.instr_reward, 'center', setup.yCen*0.8, color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);
    ...
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');
    DrawFormattedText(w, setup.instr_cat_txt, 'center', 'center', color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);    
...
%     end
    ...
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');
    DrawFormattedText(w, setup.instr_score_txt , 'center', setup.yCen, color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);
    ...
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');
    DrawFormattedText(w, setup.instr_fast, 'center', 'center', color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);
    Screen('Flip', w);
    WaitSecs(1);
    if SMsettings.settings.do_fmri
        while true
    ...
    %Questions & Start
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');
    DrawFormattedText(w, setup.instr_start_txt, 'center', 'center', color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);
    Screen('Flip', w);
    WaitSecs(1);
    if SMsettings.settings.do_fmri
        while true
    ...
    Screen('TextSize',w, round(wRect(4)/30)); Screen('TextFont',w,'Arial');    
    DrawFormattedText(w, setup.instr_repeat_txt, 'center', 'center', color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);
    Screen('Flip', w);
    WaitSecs(1);
    if SMsettings.settings.do_fmri
        while true
...
for nblock = 1:setup.nblocks
    ...
    for ntrial = 1:setup.ntrials
        ...
        if SMsettings.settings.do_fmri
            ...
            play_spin_vid(w, wSM, spin_vid, Beh_timings.interval, output.choice(ntrial,nblock), setup.r_options, setup.option_colours, setup.option_codes, time_stamp_interval, setup.flip_interval, setup.border_size);
        end
...
%Calculate payout
payout.money = output.allmoneyscore/200;
if payout.money < 0
    payout.money = 0;
end
payout.food = output.allfoodscore/100*50;
if payout.food < 0
    payout.food = 0;
end
...
DrawFormattedText(w, task_result, 'center', 'center', color.black, 60, setup.flip_flag_horizontal, setup.flip_flag_vertical);
Screen('Flip',w);
...
disp(append(num2str(payout.food),' Kilokalorien und ', num2str(payout.money),' Euro ausbezahlen.'))
...
if do_fmri 
    ...
    score_rect = OffsetRect(score_rect, full_rect(3)*0.05, full_rect(4)-RectHeight(score_rect)); % center according to score bar
    if exist('aftertrial')
    score_rect_2 = CenterRect(score_rect_2, wRect); %rightleft
    score_rect_2 = OffsetRect(score_rect_2, 0, +wRect(4)/16); % center according to score bar
    end
else
    ...
    if exist('aftertrial')
    score_rect_2 = CenterRect(score_rect_2, wRect); %rightleft
    score_rect_2 = OffsetRect(score_rect_2, 0, +wRect(4)/16);
    end
end
if exist('aftertrial')
    if aftertrial ~=0
...
function [actualTime, first_flip, end_time_interval,time_lastFlip] = play_spin_vid(w, wSM, spin_vid, iduration, choice, r_options, option_colours, option_codes, time_lastFlip, flip_interval, border_size, setup, SMsettings, wRect, blocktype_texture, diffscore, color, score, nblock, ntrial, output)
