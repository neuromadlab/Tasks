...
if settings.do_fmri == 1
    ...
    part_file = sprintf('Order_TUE004_fMRI/FCRcond_mat_TUE004_fmri.mat');

else
    
    if settings.do_NF == 0

        part_file = sprintf('Order_TUE004/FCRcond_mat_TUE004_%06d.mat',subj.id);
   
    else
        
        part_file = sprintf('Order_TUE004_NF/FCRcond_mat_TUE004_%06d.mat',subj.id);
...
if strcmp(subj.study, 'TUE002')
    ...
    p_trials = 80; %as used in tVNS study
...
timestamps.exp_on = GetSecs;
   ...
   %load jitters and initialize jitter counters
    ...
    elseif (settings.do_fmri == 0 && settings.do_NF == 0)
        load('DelayJitter_mu_0.70_max_4_trials_160.mat'); %For TUE004
    else
...
for rep = start_rep : end_rep
    ...
    for i_trial = 1 : p_trials
        ...
        if settings.do_NF == 0
            ...
            image_path = sprintf('Stimuli/%s', design.rand.image_file{i_trial,rep});

        else 
            NF_clmn_index = rep - 4;
            image_path = sprintf('Stimuli/%s', design.rand.image_file{i_trial,NF_clmn_index});
        end
       ...
       %% Fixation cross fix1 (for fMRI: not within block)
        ...
        if settings.do_fmri == 0 || block_status == 0
            ...
            timestamps.jitter_fix1(i_trial,rep) = jitter(count_jitter);
        ...
        if ~isempty(call_scale)
           ...
           if settings.do_fmri == 0
                ...
                if flag_resp == 1
                    ...
                    WaitSecs((3-timestamps.RT.scales.all(i_trial,1))+jitter(count_jitter));
           ...
           %skip index increase for last trial
           if i_trial < p_trials
                count_jitter = count_jitter + 1;
           end
...
if settings.do_fmri == 1
    ...
    if coinflip <= 0.5 % choose block x
        output.lottery.effort_winning_trial = output.data_mat(20, 12);
    else % choose block y
