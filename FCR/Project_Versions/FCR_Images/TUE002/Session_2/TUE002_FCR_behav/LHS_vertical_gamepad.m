...
if preset ~= 1
    ...
    feedback_delay = 1.5; % specifies the duration that the confirmed rating will be display on the screen
...
%--- Start trial---
%while (GetSecs - t_scale_trigger) < max_dur_rating; %number of questions within the loop
if settings.lang_de == 1
    trial.question = 'mögen';
else
...
if settings.lang_de == 1
    anchor.a1 = ['am allerstärksten gemochte Empfindung, die vorstellbar ist'];
    ...
    anchor.a11 = ['am allerstärksten zuwidere Empfindung, die vorstellbar ist'];
    ...
    anchor.a6 = ['neutral'];
    anchor.a7 = ['dis' anchor_5];
    anchor.a8 = ['dis' anchor_4];
    anchor.a9 = ['dis' anchor_3];
    anchor.a10 = ['dis' anchor_2];
    ...
    while (GetSecs - starttime) < timing.max_dur_rating && flag_resp == 0 %mousebuttons(1)~=1 
        ...
        if (GetSecs > nextTime) %Sample every 0.01 seconds
            ...
            if flag_resp==1 %Terminate and record rating on left mouseclick
                output.rating.value(i_trial,1) = 100-((Slider_y_pos - Scale_offset)/Scale_height)*200; %rescaling of scale_height independent of screen resolution [0-100]
        ...
        if flag_resp==0
            output.rating.value(i_trial,1) = 100-((Slider_y_pos - Scale_offset)/Scale_height)*200;
            output.rating.label{i_trial,1} = text_freerating;
            output.rating.subm(i_trial,1) = 1;
    end
