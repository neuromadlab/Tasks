...
doBimonocularCalibration= false;
    ...
    edfFile = ['P',num2str(str2double(subj.id)),'S',num2str(str2double(subj.sess))]; % Save file name to a variable    
...
if setup.do_eytracking == 1 && do_eyelink == 1
...
for i_block = start_block:length(setup.randomization.block_id(setup.randomization.run_id==subj.run))
  ...
  if setup.do_eyetracking == 1 && do_eyelink ==1
