...
if do_eyetracking == 1
    ...
    elseif do_eyelink == 1
        ...
        % Open dialog box for EyeLink Data file name entry. File name up to 8 characters
            ...
            edfFile = ['P',num2str(subj.id),'S',num2str(subj.sess),'B',num2str(start_block)]; % Save file name to a variable
...
color_scale_background_grey = [128 128 128]; %gray
...
for i_block = start_block:length(setup.randomization.block_id(setup.randomization.run_id==subj.run))
    ...
    if manual == 1
        body_settings =  matlab.net.http.MessageBody(['minIntensity=',num2str(amplitude),'&maxIntensity=',num2str(amplitude),'&impulseDuration=400&frequency=',num2str(freq),'&stimulationDuration=',num2str(20),'&pauseDuration=',num2str(25)]);
    else
        body_settings =  matlab.net.http.MessageBody(['minIntensity=',num2str(amplitude),'&maxIntensity=',num2str(amplitude),'&impulseDuration=400&frequency=',num2str(freq),'&stimulationDuration=',num2str(stim_length),'&pauseDuration=',num2str(25-stim_length)]);
...
if do_eyelink && do_eyetracking
    ...
    transferFile(edfFile,filename, dummymode);
end
...
function transferFile(edfFile, filename, dummymode)
        try
            if dummymode == 0 % If connected to EyeLink
